...
#include "lsc/analyzer/HighlightEngine.h"
...
void MainWindow::setupAndConnectDocks()
{
    ...
    LOG_DEBUG() << "Dock layout: before files dock";
    addDockWidget(Qt::RightDockWidgetArea, m_filesDock);
    LOG_DEBUG() << "Dock layout: after files dock";

    // ===== LSC: 直播录制 + AI 分析面板 =====
    m_livestreamDock = new LivestreamDock(this);
    m_livestreamDock->setObjectName("LivestreamDock");
    m_livestreamDock->toggleViewAction()->setShortcut(
        QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_L));
    ui->menuView->addAction(m_livestreamDock->toggleViewAction());
    ...
    m_analysisDock->toggleViewAction()->setShortcut(
        QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_A));
    ui->menuView->addAction(m_analysisDock->toggleViewAction());
    addDockWidget(Qt::RightDockWidgetArea, m_analysisDock);
    m_livestreamDock->show();
    m_livestreamDock->raise();
    m_analysisDock->show();

    HighlightEngine* lscEngine = m_livestreamDock->session()->highlightEngine();
    if (!lscEngine) {
        lscEngine = new HighlightEngine(this);
        lscEngine->setAutoExport(true, QDir::homePath() + QStringLiteral("/Videos/LiveClips"));
        lscEngine->setStrategy(HighlightEngine::createGenericStrategy(lscEngine));
        m_livestreamDock->session()->setHighlightEngine(lscEngine);
    } else if (!lscEngine->currentStrategy()) {
        lscEngine->setStrategy(HighlightEngine::createGenericStrategy(lscEngine));
    }
    m_analysisDock->setHighlightEngine(lscEngine);
    ...
    connect(m_livestreamDock, &LivestreamDock::recordingFinished,
        ...
        statusBar()->showMessage(
            QString::fromUtf8("录制完成，已导入: %1").arg(path), 5000);
        m_analysisDock->onRecordingComplete(path);
    });

    auto revealExportedClip = [this](const QString& filePath) {
        if (filePath.isEmpty()) {
            return;
        }
        showInFiles(filePath);
        statusBar()->showMessage(
            QString::fromUtf8("高光片段已导出: %1").arg(filePath), 5000);
    };
    auto appendExportedClipToPlaylist = [this](const QString& filePath) {
        if (filePath.isEmpty() || !QFileInfo::exists(filePath) || !m_playlistDock
            || !m_playlistDock->model()) {
            return;
        }
        ...
        Mlt::Producer* playlistProducer = MLT.setupNewProducer(&producer);
        if (!playlistProducer) {
            return;
        }

        playlistProducer->set(kShotcutSkipConvertProperty, true);
        playlistProducer->set(
            kPlaylistIndexProperty,
            playlist() ? playlist()->count() : m_playlistDock->model()->rowCount());
        m_playlistDock->show();
        m_playlistDock->raise();
        undoStack()->push(new Playlist::AppendCommand(*m_playlistDock->model(),
                                                      MLT.XML(playlistProducer),
                                                      false));
        m_recentDock->add(filePath.toUtf8().constData());
        delete playlistProducer;
    };
    connect(m_livestreamDock, &LivestreamDock::clipExported,
            this,
            [revealExportedClip, appendExportedClipToPlaylist](const QString& filePath,
                                                               const QString&) {
                revealExportedClip(filePath);
                appendExportedClipToPlaylist(filePath);
            });
    connect(m_analysisDock,
            &AnalysisDock::clipExported,
            this,
            [revealExportedClip, appendExportedClipToPlaylist](const QString& filePath) {
                revealExportedClip(filePath);
                appendExportedClipToPlaylist(filePath);
    ...
    connect(m_analysisDock, &AnalysisDock::highlightSelected,
        ...
        const int startMs = static_cast<int>(startSec * 1000);
        const int endMs = static_cast<int>(endSec * 1000);
        const int startFrame = qRound(MLT.profile().fps() * startSec);
        const int endFrame = qRound(MLT.profile().fps() * endSec);
        MLT.setIn(startFrame);
        MLT.setOut(endFrame);
        MLT.seek(startFrame);
        if (m_player) {
            m_player->setIn(startFrame);
            m_player->setOut(endFrame);
            m_player->seek(startFrame);
        }

        if (m_timelineDock && m_timelineDock->markersModel() && MLT.producer()) {
            if (m_timelineDock->markersModel()->markerIndexForRange(startFrame, endFrame) < 0) {
                Markers::Marker marker;
                marker.start = startFrame;
                marker.end = endFrame;
                marker.text = QString::fromUtf8("高光 %1 - %2")
                                  .arg(QTime::fromMSecsSinceStartOfDay(startMs).toString("mm:ss"))
                                  .arg(QTime::fromMSecsSinceStartOfDay(endMs).toString("mm:ss"));
                marker.color = QColor(QStringLiteral("#f39c12"));
                m_timelineDock->markersModel()->append(marker);
            }
            m_markersDock->show();
            m_markersDock->raise();
        }
    ...
    if (auto audioWaveformDock = findChild<QDockWidget *>("AudioWaveformDock")) {
        splitDockWidget(m_recentDock, audioWaveformDock, Qt::Vertical);
    } else {
        LOG_WARNING() << "Dock layout skipped AudioWaveformDock split because dock was not found";
    }
    if (audioMeterDock) {
        splitDockWidget(audioMeterDock, m_recentDock, Qt::Horizontal);
    } else {
        LOG_WARNING() << "Dock layout skipped AudioPeakMeterDock split because dock was not found";
    }
    ...
    if (m_livestreamDock) {
        tabifyDockWidget(m_subtitlesDock, m_livestreamDock);
    }
    if (m_analysisDock) {
        tabifyDockWidget(m_filesDock, m_analysisDock);
    }
    m_recentDock->raise();
    resetDockCorners();
}
