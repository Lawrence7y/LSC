...
> 状态（2026-09-10）：**B1/B3 已备好候选池但被"标签真值"卡住**（见下方附注）；B2 依赖 B1 重训；
> B4 与 B1 可合并考虑。

| # | 待办 | 说明 |
| :--- | :--- | :--- |
| B1 | **补"回放中的实战镜头"训练样本** | 当前 `replay` 类样本以"回放转场/水印"为主，缺实战镜头回放 → 这是模型失效的**根本成因**。候选池已建（见附注） |
| B2 | 下调 `replay` 阈值 0.77 → **0.70** | 证据见 §1.5；**须在 B1 重训后重新推导**（§1.5 实测：安全窗口仅约 0.11 宽，绑死当前权重） |
| B3 | **补 `replay` 测试集** | `datasets/valorant_phase_broadcast`：train 1116 / val 27 / **test 0** → 训练流程无法回归。候选池已建（见附注） |
| B4 | 考虑把 **OCR 回放证据**作为模型的补充输入 | OCR 抓得到模型漏掉的实战镜头回放，两者互补 |

#### B 附注：B1/B3 的候选池已备好，但"标签真值"是硬卡点（2026-09-10）

**① 缺口已量化**：跨 10 场录像的 22 段 OCR 回放段（共 305s）采样 70 帧实测——
模型**判为 `replay` 的帧 0/70（0.0%）**、`p_replay` 超阈值 0/70，其中 **29/70（41.4%）判为 `combat`**。
即模型在 OCR 认定的回放段上召回为 **0**。

**② 但这个数字是被污染的，且暴露了更根本的问题**：OCR 的 `replay_segments` 启发式是
"计时器不可读"，而**中场休息/广告/转场同样"计时器不可读"**。抽查 2 段即有 1 段证伪：

| 抽查帧 | OCR 段 | 视觉核验结论 |
| :--- | :--- | :--- |
| `12-00-36` @620s（§1.3 曾称"目视确证 REPLAY 水印"） | 判为回放 | **无明确回放字样**；画面为第一人称交战镜头 + 选手摄像头叠加（`CHICHOO 4/0/0`）、比分 EDG 2:0 TE、计时器 0:55 |
| `18-05-24` @1003s | 判为回放 | **不是回放**，是**中场休息（HALFTIME）转场**（叠层 `00:00:50` 倒计时、比分 6-6） |

→ **`replay_segments` 不能作为 B1/B3 的标注真值**（会把中场休息/广告标成 `replay`）；
→ 因此上文 41.4% 的"判 combat"是**上限式估计**，不能直接当结论（分母含非回放内容）；
→ 唯一可信路径是**逐帧目视标注**——这正是 §1.3 做对照实验时的方法。计划里 B1/B3 标注为
"周期最长"，原因即在此。

**③ 候选池已就绪**（可直接交给标注者）：

- 44 张候选帧（22 段 × 2 帧，25%/75% 位），含完整溯源：会话 / 精确时间戳 / 所属回放段 /
  `round_key` / 原录像路径；
- 逐帧附模型预测，便于聚焦核验：**`combat 20 / non_game 14 / result 6 / buy 2 / unknown 2`，
  无一张被判 `replay`**；
- 清单文件：`C:/lsc_tmp/verify/pool/review_sheet.tsv`（含空的"人工判定"列）；
- **优先核验对象是模型判 `combat` 的那 20 张**（`p_combat` 0.688–0.870、`p_replay` 0.003–0.173）
  ——它们若确为回放，就正是 B1 要补的"回放中的实战镜头"。

**④ 卡点**：本会话的视觉核验工具在过程中变为不可用（`mimo_vision` 报 unknown tool、
`read_image` 报当前模型不支持图像输入、`dev_tool_search` 搜不到视觉工具），且当前模型
（`deepseek-flash`）不声明图像输入 → **预筛无法由本轮执行**，需由具备视觉能力的标注者
（或换用支持图像的模型）完成人工确认后再入集。

**入集方式**（待标注完成后）：帧写入 `datasets/valorant_phase_broadcast/test/replay/`（测试集
≥30 帧，满足 B3 验收）与 `train/replay/`（B1 训练样本），并补 `manifest_*.jsonl` 条目
（字段契约见 `scripts/valorant_vision/manifest_broadcast.jsonl`）。注意
`datasets/valorant_phase_broadcast*/` 已在 `.gitignore:201` 中，属本地数据、不进仓库。
