...
> 状态（2026-09-10）：**B1/B3 已建成 80 帧水印确证数据集**（客观真值，非猜测）；
> **B2 被实测推翻**（见附注 ③，不可单独执行）；**新增 B5**（融合稀释回放信号，可操作）；
> B4 与 B1 可合并考虑。

| # | 待办 | 说明 |
| :--- | :--- | :--- |
| B1 | **补"回放中的实战镜头"训练样本** | 已在 80 帧水印确证集上证实：模型 `replay` 召回 **0.0%**，八成判成 `non_game` → 重训是唯一真解（见附注 ②） |
| B2 | 下调 `replay` 阈值 0.77 → **0.70** | ❌ **实测推翻**：在 80 帧真实回放上 0.70 仅召回 11.2%（§1.5 的 val 集不能代表真实转播）。**须在 B1 重训后于确证集上重新推导**（见附注 ③） |
| B3 | **补 `replay` 测试集** | **80 帧水印确证帧已就绪**（≥30 帧要求已满足），待人工抽检后入集（见附注 ①⑥） |
| B4 | 考虑把 **OCR 回放证据**作为模型的补充输入 | OCR 抓得到模型漏掉的实战镜头回放，两者互补 |
| **B5** | **修复 `replay` 类的融合稀释**（新增） | `top_HUD 0.3` 看不到底部回放水印，把整帧 0.96–0.99 的置信度拉到阈值下。改为对 `replay` 用整帧判定（或取 `max`／换底部水印带）→ 同一批 80 帧 ≥0.77 的检出从 **0/80 升到 17/80**。**增益项，不能替代 B1**（见附注 ④） |

#### B 附注：B1/B3 已产出**水印确证**数据，并推翻 §1.5 的 B2 结论（2026-09-10 实测）

**① 用仓库自带 OCR 建立客观真值**（绕开"视觉模型不可用"）：回放水印是**文字**，故对
22 段 OCR 回放段按秒/半秒密抽帧（扫描 301→ 扩样共约 600 帧），用 `ocr_detector` 逐帧搜
`REPLAY/回放/重播/慢动作`，**只保留 OCR 真读到字面水印的帧** → **80 帧水印确证回放帧**
...
| ---: | ---: |
| **0.77（现状）** | **0 / 80（0.0%）** |
| 0.70（B2 原推荐） | 9 / 80（**11.2%**） |
| 0.60 | 15 / 80（18.8%） |

模型分布：`non_game` 52 / `unknown` 28 / **`replay` 0**。即**面对明确写着 REPLAY 的画面，
模型一次都没认出来**，八成判成"非游戏"。→ **B1（补样本重训）是唯一真解**。

**③ ⚠️ 推翻 §1.5 的 B2 结论**：§1.5 在 **val 集**（27 帧，以"回放转场/水印"为主）上测得
"0.70 → 召回 100%"，据此把阈值下调列为 B2。但**在真实赛事回放上 0.70 只有 11.2%**——
**val 集不能代表真实转播回放**。→ **B2 不可单独执行**，必须在 B1 重训后于**本附注的
80 帧确证集**上重新推导（该集正可作为 §7.3 要求的"另一段视频复核"载体）。

**④ 🔴 新发现（可操作）：`top_HUD` 融合在稀释回放信号**。
广播档融合为 `full 0.7 + top_HUD 0.3`，而**回放水印在画面底部**，top 34% 裁剪看不到它，
0.3 权重把整帧的高置信度硬拉到阈值以下：

| 同一批 80 帧 | p_replay 均值 | 最大 | ≥0.77 |
| :--- | ---: | ---: | ---: |
| 融合（现行） | 0.194 | 0.750 | **0 / 80** |
| 仅整帧 | 0.266 | **0.9998** | **17 / 80** |

例：`02-06-12_473750` 融合 0.720 → 仅整帧 **0.998**；`02-06-12_472250` 0.679 → **0.962**。
→ 存在**低成本改进**：对 `replay` 类改为整帧判定（或对 `replay` 取 `max(融合, 整帧)`），
或把融合的第二路从"顶部 HUD"换成"底部水印带"。已新增为 **B5**（见下表）。
注：仅整帧在 0.77 下也只到 17/80，故 B5 是增益项、**不能替代 B1**。
