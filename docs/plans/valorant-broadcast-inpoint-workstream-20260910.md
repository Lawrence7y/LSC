...
#### ✅ B1 已收口：改标签重训无效 → 真因是"输入尺度看不见标记" → **标记 ROI 支路达标**（2026-09-11）

完整报告：`docs/reports/valorant-broadcast-marker-roi-branch-20260911.md`。

**根因（否定性测量）**：把标记区按原生分辨率裁出放大到 224×224 喂给现有模型，
标记帧的 `p_replay` 只有 **0.002**、比无标记对照（0.10）还低 —— 放大后的裁图对模型是
**彻底 OOD**。配合几何测量（224×224 下标记仅 ≈20×8px、占整帧 8% 的边缘能量），
结论是：**不是标签错、不是阈值错，是信号没进模型**。

**方案**：模型元数据声明 `marker_roi_branch`，把标记区按原生分辨率裁成**独立输入支路**，
回放证据取 `max(整帧, 各 ROI 最大值)`。四条不可退让的性质：默认关闭（未声明时行为逐字不变）、
只增不减（取 max 不取加权平均）、缺文件不中断链路、放大口径与离线一致（CUBIC，有源码守卫）。
`weight` 是证据乘子（1.0 = 恒等）。

**结果（同一主模型、只换支路权重文件）**：

| | val Macro F1 | val replay 召回 | val replay 精确 | combat 召回 | test replay 召回 | test replay 精确 |
| :--- | ---: | ---: | ---: | ---: | ---: | ---: |
| baseline | 0.8935 | 0.9000 | 1.0000 | 0.9490 | 0.3438 | 1.0000 |
| 只加支路（4 会话） | 0.8752 | 1.0000 | 0.8824 | 0.9345 ↓ | 0.9688 | 1.0000 |
| **+ 本地 12 录像扩样** | **0.9054** | **1.0000** | **1.0000** | **0.9490** | **0.9688** | **1.0000** |

→ 要求达成：`test/replay 召回 0.9688 ≥ 0.95` 且 `val Macro F1 0.9054 > 基线`，**逐类指标零附带损伤**。

**四路扩展（同日完成）**：扩样时发现**第四种标记位置"顶部居中"**（2026 进化者杯，
只有两个框时该整段命中 0/1200）。把 ROI 扩成四路并整套重跑后，**指标逐位不变**
（val 0.9054 / test replay 召回 0.9688），而逐 ROI 检出率：`top_center` **0%→100%**、
`bottom_right` **63%→99%**、`top_right` 95%→100%，各 ROI 负样本 0 误报。
`top_left_small` 仍为惰性 ROI（逐裁剪 OCR 读不出 ≈20×5px 的字形，正样本 0），
修法为"整帧 OCR 取框 + 按几何归属"，本轮未做。详见该报告 §9。

**真实直播验收**（新程序 `verify_broadcast_replay.py`，huya `29701502` 实况 180 秒 / 90 帧）：
与独立 OCR 通道**一致 98.89%、误报 0、漏报 1** → **PASS**。

**顺带确立的三条事实（都会影响后续设计）**：
1. 回放标记有**四种**互斥位置：右上角 / **顶部居中**（2026 进化者杯）/ 顶部偏左小字 / 右下角
   （另有"居中大字"整屏转场，字够大不需 ROI）——B5 的"水印在画面底部"只对其中一种成立；
2. **会话多样性是泛化瓶颈**：只加支路时 7 个 val 误报**全部来自同一个会话**，
   加 12 个本地录像后全部消失 → 后续加素材优先加**不同转播方**；
3. 标注口径必须是**逐裁剪 OCR**（按帧标签打标会让约一半正样本里根本没有标记）；
   且 640×360 帧下"顶部偏左小字"仅约 20×5px，**固有不可标**。
