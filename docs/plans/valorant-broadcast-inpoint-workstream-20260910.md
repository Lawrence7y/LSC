...
| **第 2 步进度** | ✅ 2.1（A5+A6，`268747e`）、2.2（A4+A3，`f455339`）、2.3（A2 收窄版，`08666b7`）均已完成；⬜ **仅剩 2.4 / A7**（前导回放前移，并承接 A2 的"不可达部分"） |
...
> 状态（2026-09-10）：A1 ✅已决策不接线 / A2 ✅已完成（范围已收窄，见 §2.1） / A3+A4 ✅已完成（`f455339`） / A5+A6 ✅已完成（`268747e`） / **A7 ⬜未做**
...
| A7 | 非切块候选允许"前导回放"时**前移起点**（而非整条拒绝） | `valorant_broadcast.py:702` | ⬜ 未做（中：可能引入重复回合）；A2 的"不可达部分"也落在这里，见 §2.1 |
...
| 2.4 | ⬜ **A7**（前导回放前移） | 无重复回合；`round_key` 去重仍生效；同时承接 A2 的"不可达部分"（前置回放检测） |
