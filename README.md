...
- **放大预览控制条一体化 + 自动隐藏**：时间线与走带按键合成同一块玻璃面板；默认向下隐藏，鼠标经过滑出、静止 2.5s 自动收起（拖动时间线 / 画质下拉打开期间钉住，键盘焦点由 `:focus-within` 兜底）
- **窗口播放（原生画中画）**：放大态控制条新增「窗口播放」按钮，一次点击 = 缩小回网格 + 进入系统原生画中画小窗，可跨应用常驻，退出小窗不重新放大
- **回放交互口径对齐（能点的范围 = 真能回放的范围）**：放大预览条可点/可拖范围改为真实缓冲范围，回放设置只作上限；新增 2s 边界容差，分片边界/浮点误差不再把一次普通点击推给重量级的本地文件回看通道
- **移除放大预览区误导性时长文案**：删除「回看设置 X · 实际可回放 Y」，主时间线紫标（真实起点 / 真实可回放量）不变

### v1.0.16（2026-09-13）

- **界面重设计**：工作台浅色 + 品牌色 `#31B3AE` 主题统一，房间卡片布局简化（徽章归入头部与元数据行），新增通知岛与状态标签体系，分析进度与导出摘要一目了然
- **官方解说分支性能增强**：在线微步骤与媒体预算约束优化；标记支路推理异常自动隔离（DML OOM 不再拖垮主链路）；A5 幅度上限按实测放宽，修复误裁
- **设置缓存毒化（P0）**：修复保存设置后所有连接失效的崩溃
- **本地文件回看**：定位死线改为停滞看门狗，出画前读取块放大 4× 加速追赶
- **切片标签撞号**：改用 per-room 单调计数器，跨回合不再撞号
- **草稿重叠对账**：收尾补扫的重叠候选进 `excluded_clips`，`skipped_unaccounted` 恢复恒等 0
- **平台抓取**：统一默认直连，死代理不再阻断解析
- **安装包隐私加固**：打包不再携带任何本地配置、录制历史与日志

### v1.0.15（2026-09-11）

- **草稿静默丢弃清零**：终态权威快照跨会话保留、扫描通路终态补投影、权威注册表接线修复，已定稿切片不再丢失
- **跳过原因可辨**：结构化 `reason_code`（`END_NOT_FINAL` / `NEVER_AUDITED` / `OVERLAP_DEDUP` 等），切片列表逐条标注「为什么没进草稿」
- 同名草稿不再互相覆盖；收尾完成判定收紧（C6）；新增离线复算工具与 L3 真实环境验收全绿

### v1.0.14（2026-09-11）

- **导出门禁对齐后端**：赛事切片导出不再被「必须人工确认」多余拦截（两端判据钉死，回归夹具保护）

### v1.0.13（2026-09-11）

- **官方解说分支审计存活**：取消路径结论不丢、预算耗尽降级留队不交半成品、超长候选分裂推进与在线预取媒体预算约束、审计任务卡死可观测

### v1.0.12（2026-09-10）

- **持续分析 → 剪映草稿链路修复**：新录制 epoch 隔离（旧会话切片不再混入）、审计拒绝终态同步清理（被拒回合不再混入草稿）、草稿生成三重权威校验
