...
*   **持续分析漂移复核**：多房持续分析每 `10` 分钟尝试重新采集 `8s` 预览音频。目标房未进入 DVR 深度回看时即运行（**不要求窗口失焦/隐藏**——旧实现要求前台交互时跳过，实测全程聚焦导致复核永不触发、偏移长期过期）；复核前各房独立回到自己的直播沿，完成后重新应用 offset。分析进入**收尾（finalizing）**阶段时还会立即触发一次复核，保证尾部回合入列即用最新偏移。
*   **预览轴锚点（公共轴一致性）**：`preview_to_common_delta = (align_mono - 预览 PTS currentTime) + 房间相对偏移`；`recording_to_common_delta = media_start_mono + 房间相对偏移`（**只含相对偏移，禁止叠加锚点**，否则录制轴双重偏移）。MSE 预览流 currentTime 基座是流相关任意值，与录制轴的 `media_start_mono` 基座通常相差数百~数千秒；不锚定会导致公共轴上播放头与切片错位、点击切片 seek 出缓冲范围。旧前端/旧数据缺 `preview_current_time` 时自动退化为纯相对偏移。
