...
import { PlusOutlined, VideoCameraOutlined, SoundOutlined, MutedOutlined, AlignLeftOutlined, ReloadOutlined } from '@ant-design/icons'
...
export default function Workbench() {
  const { isConnected, connectionStatus, send, on, reconnect } = useWebSocket()
  const rooms = useAppStore((state) => state.rooms)
  const selectedRoomId = useAppStore((state) => state.selectedRoomId)
  const settings = useAppStore((state) => state.settings)
  // 连接状态提示：任何非 connected 状态都显示，connected 时隐藏
  // connecting 延迟 5 秒显示（避免短暂重连闪烁），disconnected/reconnect_failed 延迟 1 秒
  ...
  useEffect(() => {
    ...
    const delay = connectionStatus === 'connecting' ? 5000 : 1000
    const timer = setTimeout(() => setShowDisconnectAlert(true), delay)
  ...
  // 导出进度跟踪：job_id -> {percent, elapsed, total}
  const [exportingJobs, setExportingJobs] = useState<Record<string, {percent: number; elapsed: number; total: number}>>({})
  ...
  // 房间级操作防抖：跟踪每个房间正在进行的操作，防止快速重复点击发送多个请求
  const pendingRoomOpsRef = useRef<Map<string, number>>(new Map())
  const _isRoomOpPending = useCallback((roomId: string, op: string): boolean => {
    const expiry = pendingRoomOpsRef.current.get(`${roomId}:${op}`)
    return expiry !== undefined && Date.now() < expiry
  }, [])
  const _setRoomOpPending = useCallback((roomId: string, op: string, timeoutMs: number = 5000) => {
    pendingRoomOpsRef.current.set(`${roomId}:${op}`, Date.now() + timeoutMs)
    setTimeout(() => pendingRoomOpsRef.current.delete(`${roomId}:${op}`), timeoutMs)
  }, [])
  ...
  useEffect(() => {
    const id = setInterval(() => {
      ...
      for (const rid of Object.keys(registry)) {
        ...
        // 防御性清理：如果 video 元素已脱离 DOM（组件卸载但 cleanup 未执行），移除泄漏的引用
        const videoEl = entry?.player?.videoElement as HTMLVideoElement | undefined
  ...
  useEffect(() => {
    ...
    unsubs.push(on('clip_completed', (data: any) => {
      ...
      if (data?.room_id && typeof data.start === 'number' && typeof data.end === 'number') {
        ...
        const updatedClips = store.clips.map(c => {
          // 优先用 job_id 匹配，回退到 start+end+room_id
          const match = (jobId && c.job_id === jobId) ||
            (c.start === data.start && c.end === data.end && c.room_id === data.room_id)
          return match ? { ...c, exported: true, outputPath: data.output_path } : c
        })
    ...
    // 导出进度上报：更新 exportingJobs 状态，驱动 ClipList 进度条
  ...
  const handleDisconnect = useCallback((roomId: string) => {
    if (_isRoomOpPending(roomId, 'disconnect')) return
    _setRoomOpPending(roomId, 'disconnect')
    send('disconnect_room', { room_id: roomId })
  }, [send, _isRoomOpPending, _setRoomOpPending])

  // 切换静音（乐观更新：立即翻转本地状态再发 WS，避免后端节流导致 UI 延迟）
  const handleToggleMute = useCallback((roomId: string) => {
    ...
    // 乐观更新 store，房间卡片静音图标立即响应
    useAppStore.getState().updateRoom(roomId, { preview_muted: newMuted })
    send('set_preview_muted', { room_id: roomId, muted: newMuted })
  }, [send])

  // 开始录制（加防抖，避免快速重复点击同时启动多路录制）
  const handleStartRecord = useCallback((roomId: string) => {
    if (_isRoomOpPending(roomId, 'startRecord')) return
    _setRoomOpPending(roomId, 'startRecord')
    send('start_recording', { room_id: roomId })
  }, [send, _isRoomOpPending, _setRoomOpPending])

  // 停止录制（加防抖）
  const handleStopRecord = useCallback((roomId: string) => {
    if (_isRoomOpPending(roomId, 'stopRecord')) return
    _setRoomOpPending(roomId, 'stopRecord')
    send('stop_recording', { room_id: roomId })
  }, [send, _isRoomOpPending, _setRoomOpPending])

  // 启用/停止预览（Electron 模式：后端 FFmpeg 抓帧推送）
  const handleTogglePreview = useCallback((roomId: string, enabled: boolean, quality?: string) => {
    send('enable_preview', { room_id: roomId, enabled, mode: 'mse', preview_quality: quality })
  }, [send])
  ...
  const mseSeek = useCallback((roomId: string, time: number) => {
    ...
    if (video) {
      if (video.buffered.length > 0) {
        ...
        } else if (time < bufStart) {
          // 超出可回看范围：不强制跳转，提示用户
          const seekableSec = Math.floor(bufEnd - bufStart)
          message.warning(`该时间点已超出回看范围（可回看最近 ${seekableSec} 秒）`)
        } else {
          // seek 到未来位置（直播边缘之后）：跳到直播最新位置
  ...
  const handleAddClip = useCallback((roomId: string) => {
    ...
    if (room && room.mark_in !== null && room.mark_out !== null) {
      if (room.mark_in >= room.mark_out) {
        message.warning('入点必须早于出点，请重新标记')
        return
      }
  ...
  // 一键对齐：将所有选中房间的播放位置同步到统一时间点
  // 取所有选中房间中最小的缓冲区末端作为对齐目标，确保所有房间都能 seek 到该位置
  const handleAlignPreviews = useCallback(() => {
    if (selectedRoomIds.size === 0) return
    const registry = (window as any).__msePlayers
    if (!registry) return
    let minBufEnd = Infinity
    selectedRoomIds.forEach(rid => {
      const video = registry?.[rid]?.player?.videoElement as HTMLVideoElement | undefined
      if (video && video.buffered.length > 0) {
        const bufEnd = video.buffered.end(video.buffered.length - 1)
        if (bufEnd < minBufEnd) minBufEnd = bufEnd
      }
    })
    if (minBufEnd === Infinity) return
    const targetTime = Math.max(0, minBufEnd - 1)
    selectedRoomIds.forEach(rid => {
      const video = registry?.[rid]?.player?.videoElement as HTMLVideoElement | undefined
      if (video) {
    ...
    message.success(`已对齐 ${selectedRoomIds.size} 个直播间到统一位置`)
  }, [selectedRoomIds])
  ...
  // 打开导出的文件
  const handleOpenExportFile = async (outputPath: string) => {
    if (window.electronAPI) {
      try {
        const result = await window.electronAPI.openPath(outputPath)
        if (result && !result.success) {
          message.error(`无法打开文件：${result.error || '路径被拒绝'}`)
        }
      } catch (e) {
  ...
  // 取消正在进行的导出任务
  const handleCancelExportJob = useCallback((jobId: string) => {
  ...
  const handleStartAnalysis = () => {
    ...
    setAnalyzing(true)
    send('start_analysis', { room_id: room.room_id, threshold: 0.3 })
  }
  ...
  // ── 导出文件操作 ──
  const handleOpenExportFolder = async (outputPath: string) => {
    if (window.electronAPI) {
      try {
        if (window.electronAPI.showItemInFolder) {
          const result = await window.electronAPI.showItemInFolder(outputPath)
          if (result && !result.success) {
            // showItemInFolder 被拒绝时回退到 openPath 打开所在目录
            const dir = outputPath.replace(/[/\\][^/\\]+$/, '')
            await window.electronAPI.openPath(dir)
          }
        } else {
          await window.electronAPI.openPath(outputPath.replace(/[/\\][^/\\]+$/, ''))
        }
      } catch (e) {
  ...
  // 点击切片：选中房间并跳转到入点
  // 多选时取第一个选中房间作为代表，确保 ControlBar/Timeline 有数据可用
  const selectedRoom = rooms.find(r => r.room_id === selectedRoomId)
    ?? (selectedRoomIds.size > 0 ? rooms.find(r => selectedRoomIds.has(r.room_id)) : undefined)
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      {showDisconnectAlert && (
        <Alert
          type={connectionStatus === 'reconnect_failed' ? 'error' : 'warning'}
          message={
      ...
      <div style={{ 
        ...
        <Space>
          ...
          <Button
            ...
            loading={analyzing}
            disabled={!selectedRoom || !selectedRoom.record_output_path}
          >
            {analyzing ? '分析中...' : '分析高光'}
          </Button>
        </Space>
            ...
            onClick={() => {
              ...
              rooms.forEach(r => {
                if (r.preview_enabled) {
                  // 乐观更新 store，所有房间卡片静音图标立即同步
                  store.updateRoom(r.room_id, { preview_muted: newMuted })
      ...
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
                ...
                {sortedRooms.map(room => (
                  <Col key={room.room_id} xs={24} sm={12} lg={12} xl={8}>
                    <RoomCard
                      ...
                      previewQuality={settings.preview_quality}
        ...
        <div style={{ 
          ...
          <ClipList
            ...
            onCancelExport={handleCancelExportJob}
            exportProgress={exportingJobs}
          />
        </div>
      </div>
