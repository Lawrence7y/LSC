...
# Analytics jobs in progress: {room_id: {"progress": 0.0, "highlights": [...]}}
...
def register_room_handlers(server, bridge):
    ...
    def _broadcast_rooms():
        task = asyncio.create_task(server.broadcast('rooms_updated', {'rooms': _rooms_list(manager)}))
        # 消费 task 异常，避免 asyncio 输出 "Task exception was never retrieved" 警告
    ...
    async def handle_connect(websocket):
        ...
        try:
            await asyncio.get_running_loop().run_in_executor(
                None, lambda: bridge.call(_restore)
            )
        except Exception as exc:
    ...
    async def handle_add_room(data):
        ...
        try:
            room = await asyncio.get_running_loop().run_in_executor(
                None, lambda: bridge.call(_add, timeout=30.0)
            )
        except TimeoutError:
    ...
    async def handle_connect_room(data):
        ...
        try:
            success = await asyncio.get_running_loop().run_in_executor(None, lambda: bridge.call(_connect))
        except Exception as exc:
            return {'success': False, 'error': humanize_error(str(exc))}
        _broadcast_rooms()
    ...
    async def handle_disconnect_room(data):
        ...
        try:
            await asyncio.get_running_loop().run_in_executor(None, lambda: bridge.call(_disconnect))
        except Exception as exc:
            return {'success': False, 'error': humanize_error(str(exc))}
        _broadcast_rooms()
        return {'success': True}
    ...
    async def handle_set_preview_muted(data):
        ...
        await asyncio.get_running_loop().run_in_executor(None, lambda: bridge.call(_set_muted))
        _broadcast_rooms()
        return {'success': True}
    ...
    async def handle_start_recording(data):
        ...
        try:
            success = await asyncio.get_running_loop().run_in_executor(None, _start)
        except Exception as exc:
            _broadcast_rooms()
            return {'success': False, 'error': humanize_error(str(exc))}

        room = await asyncio.get_running_loop().run_in_executor(None, lambda: bridge.call(manager.get_room, room_id))
    ...
    async def handle_stop_recording(data):
        ...
        try:
            success = await asyncio.get_running_loop().run_in_executor(
                None, lambda: bridge.call(_stop, timeout=30.0)
            )
        except Exception as exc:
            _broadcast_rooms()
            return {'success': False, 'error': humanize_error(str(exc))}
    ...
    async def handle_remove_room(data):
        ...
        try:
            await asyncio.get_running_loop().run_in_executor(None, lambda: bridge.call(_remove))
        except Exception as exc:
            return {'success': False, 'error': humanize_error(str(exc))}
        _broadcast_rooms()
        _persist_current_rooms(manager)
    ...
    async def handle_seek(data):
        ...
        success = await asyncio.get_running_loop().run_in_executor(None, lambda: bridge.call(_seek))
        return {'success': bool(success)}
    ...
    async def handle_toggle_play_pause(data):
        ...
        success = await asyncio.get_running_loop().run_in_executor(None, lambda: bridge.call(_toggle))
        _broadcast_rooms()
        return {'success': bool(success)}
    ...
    async def handle_export_clip(data):
        ...
        def _export():
            ...
            # 墙钟时间轴转换：将标记时的墙钟时间转换为录制文件中的秒位置。
            # recording_pos = mark_wallclock - recording_start_mono - preview_latency
            # 优先使用墙钟时间戳（精确），回退到前端传入的 start/end（兼容旧标记）
            export_start = start
            export_end = end
            if (room.recording_start_mono is not None
                    and room.mark_in_wallclock is not None
                    and room.mark_out_wallclock is not None):
                latency = getattr(room, 'preview_latency', 2.0)
                export_start = max(0.0, room.mark_in_wallclock - room.recording_start_mono - latency)
                export_end = max(0.0, room.mark_out_wallclock - room.recording_start_mono - latency)
        ...
        clip_id, error = await asyncio.get_running_loop().run_in_executor(None, lambda: bridge.call(_export))
    ...
    async def handle_cancel_export(data):
        ...
        try:
            cancelled = await asyncio.get_running_loop().run_in_executor(
                None, lambda: bridge.call(_cancel)
            )
        except Exception as exc:
            return {'success': False, 'error': humanize_error(str(exc))}
    ...
    async def handle_enable_preview(data):
        ...
        if mode == 'mse':
            return await _handle_mse_preview(server, manager, room_id, enabled)
        ...
        success = await asyncio.get_running_loop().run_in_executor(None, lambda: bridge.call(_preview))
        _broadcast_rooms()
        return {'success': bool(success)}

    # MSE preview handler
    async def _handle_mse_preview(srv, mgr, room_id: str, enabled: bool) -> dict[str, Any]:
        ...
        if enabled:
            ...
            try:
                ...
                snapshot = await asyncio.get_running_loop().run_in_executor(
                    None, lambda: bridge.call(_snapshot)
                )
                if snapshot is None:
                ...
                # Parse resolution/framerate settings to optimize transcoding
                settings = load_settings()
                width = height = 0
                resolution = settings.get('resolution', '原画')
                if resolution != '原画' and ':' in resolution:
                ...
                async def _on_mse_error(room_id: str, err: str, loop):
                    ...
                    try:
                        await loop.run_in_executor(None, lambda: bridge.call(_clear_preview))
                    except Exception as exc:
                ...
                success, error_detail = await asyncio.get_running_loop().run_in_executor(None, _start)
                ...
                try:
                    await asyncio.get_running_loop().run_in_executor(
                        None, lambda: bridge.call(_set_preview_enabled)
                    )
                except Exception as exc:
            ...
            await asyncio.get_running_loop().run_in_executor(None, lambda: bridge.call(_disable))
            _broadcast_rooms()
    ...
    async def handle_start_analysis(data):
        ...
        result = await asyncio.get_running_loop().run_in_executor(None, _do_analysis)
        return result
