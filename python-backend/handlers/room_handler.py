...
# 单房间录制轴→直播 MSE 预览轴校准保护。
_PREVIEW_CLOCK_MAX_SAMPLE_AGE_SEC = 15.0
# MSE currentTime 可能携带任意 PTS 基座，不能按“直播延迟通常几秒”拒绝；
# 这里只拦截明显的单位/系统时钟错误，允许约 24 小时的流时间基座。
_PREVIEW_CLOCK_MAX_ABS_DELTA_SEC = 24 * 60 * 60
...
def format_ai_round_clip_label(streamer: str, round_idx: int, index: int = 0) -> str:
    """AI 回合切片 label：{主播}_R{NN}。

    序号用 per-room 单调计数器 ``index``（保证同房间唯一）；``round_idx`` 是
    分析器的回合枚举序号，跨扫描批次会重复——2026-09-13 真机两场都在列表里
    出现两条 R02（不同回合同号）。index<=0 时回退 round_idx 兼容旧调用。
    """
    seq = index if index and index > 0 else round_idx
    return f"{_sanitize_streamer_name(streamer)}_R{seq:02d}"
# 每房间手动切片计数器（room_id -> 当前序号）
_manual_clip_counters: dict[str, int] = {}
# 每房间 AI 回合切片计数器（room_id -> 当前序号）
_ai_clip_counters: dict[str, int] = {}
# listed_key -> 首发分配的切片标签：upsert（边界精修重新入列）必须复用首发标签。
# 此前 upsert 直接取当前计数值，会与"最近一条首发"的切片撞号
# （2026-09-13 真机：R02 同时挂在 00:05:18-00:05:45 与 00:01:45-00:04:15 两条上）。
_listed_clip_labels: dict[str, str] = {}


def _resolve_ai_clip_label(listed_key: str, rid: str, room_name: str, round_idx: int, *, is_first: bool) -> str:
    """分配/复用 AI 回合切片标签。

    - 首发入列（is_first=True）：序号自增并按 listed_key 记忆；
    - upsert（边界精修后重新入列）：复用首发标签，避免与后续切片撞号；
    - 无首发记忆的 upsert（进程重启后）：退回当前序号，保持旧行为。
    """
    remembered = _listed_clip_labels.get(listed_key)
    if remembered:
        return remembered
    if not is_first:
        # 无首发记忆（进程重启后）：退回当前单调计数器；计数器不存在时
        # format 内部回退 round_idx（旧行为）。
        return format_ai_round_clip_label(room_name, round_idx, _ai_clip_counters.get(rid, 0))
    # 有界化与 _listed_clip_ids 同纪律（_CLIP_KEY_CACHE_MAX=20000，超限裁最旧一半）
    if len(_listed_clip_labels) >= 20000:
        for _old_key in list(_listed_clip_labels)[:10000]:
            _listed_clip_labels.pop(_old_key, None)
    ...
    _listed_clip_labels[listed_key] = label
    return label
...
# 房级「终态权威快照」：持续分析任务态被 pop 之后，导出/草稿仍需回答
# 「这条 round_key 属于当前录制 epoch 吗、它的权威终态是什么」。
# 现场（2026-09-11 20:45:20→20:45:21）：收尾任务态 pop 与用户导出相隔 1 秒，权威回落到
# 20:37 的旧分析 sidecar，把 20:43:26 已定稿的 round-000105 改回 pending_lookahead 后
# 按「未确认」跳过；round-000135 则因不在任何权威集合被报成「旧分析会话遗留切片」。
# 快照只读、不改变读路径优先级（活跃任务仍是第一权威），新录制 epoch / 删房时清除。
_last_authority_snapshots: dict[str, dict[str, Any]] = {}
_AUTHORITY_SNAPSHOT_LISTED_MAX = 500


def _preserve_authority_snapshot(
    room_id: str,
    task_state: dict[str, Any] | None,
    *,
    source: str = 'continuous_finalize',
) -> None:
    """任务态被丢弃前留下终态权威快照（切片入列快照 + 拒绝 tombstone）。"""
    if not room_id or not isinstance(task_state, dict):
        return
    listed = task_state.get('listed_clips')
    if not isinstance(listed, dict) or not listed:
        return
    kept = [
        (key, dict(value))
        for key, value in list(listed.items())[-_AUTHORITY_SNAPSHOT_LISTED_MAX:]
        if isinstance(value, dict)
    ]
    with _analysis_jobs_lock:
        _last_authority_snapshots[room_id] = {
            'room_id': room_id,
            'target_room_ids': list(task_state.get('target_room_ids') or []),
            'recording_id': str(task_state.get('recording_id') or ''),
            'listed_clips': dict(kept),
            'rejected_round_keys': dict(task_state.get('rejected_round_keys') or {}),
            'listed_clip_count': len(listed),
            'saved_at': time.time(),
            'source': source,
        }
    _log.info(
        "终态权威快照已保留: room_id=%s, listed=%d, source=%s",
        room_id, len(kept), source,
    )


def _authority_snapshot_for(room_id: str) -> dict[str, Any] | None:
    """读取房级终态权威快照（无则 None）。"""
    if not room_id:
        return None
    with _analysis_jobs_lock:
        return _last_authority_snapshots.get(room_id)


def _sync_analysis_save_path(manager: Any, room_id: str, video_path: str | None) -> str:
    """落盘前把分析结果路径同步到房间当前录像（归档改名后不再写旧名 sidecar）。

    录像定稿改名（`_录制中.mp4` → `_至_….mp4`）时 `sidecar 随录像定稿改名`，
    但持续分析循环里可能仍持有改名前的路径：现场 20:38:42 改名，20:39:27 那次
    落盘仍写 `…_录制中.analysis.json`，归档文件的 sidecar 冻在 20:37 快照。
    返回可用的保存路径（房间当前录像存在时以其为准，否则退回入参）。
    """
    fallback = str(video_path or "")
    try:
        room = manager.get_room(room_id) if manager is not None else None
    except Exception:  # noqa: BLE001 — 取房间失败不应阻断落盘
        room = None
    current = str(getattr(room, "record_output_path", "") or "")
    if not current or current == fallback:
        return fallback or current
    if os.path.isfile(current):
        if fallback and not os.path.isfile(fallback):
            _log.info(
                "分析落盘路径已随归档改名同步: room_id=%s, %s -> %s",
                room_id, os.path.basename(fallback), os.path.basename(current),
            )
        return current
    return fallback or current


def _clear_authority_snapshot(room_id: str) -> None:
    """清除终态权威快照（新录制 epoch / 删房）。"""
    if not room_id:
        return
    with _analysis_jobs_lock:
        _last_authority_snapshots.pop(room_id, None)
_VALORANT_INCREMENTAL_LOOKBACK_SEC = 30.0  # 纯 OCR 增量回看（失败/重连/首窗兜底，与 valorant_plugin 一致）
_VALORANT_STEADY_LOOKBACK_SEC = 8.0  # 稳态增量回看：FSM/锚点跨窗口持久化，回看只承担 seek 稳定性缓冲
_VALORANT_MAX_CATCHUP_SEC = 90.0  # 单次 tick 最多向前追赶的新内容时长
...
# 收尾缺口补扫产出的候选（视觉低频巡检合成）：与 OCR 候选一样入列待审，
# 但来源标记独立保留，便于事后统计"哪些切片是补扫捞回来的"。
_SWEEP_BOUNDARY_SOURCE = "valorant_vision_sweep_v1"
_LISTABLE_BOUNDARY_SOURCES = frozenset({_OCR_BOUNDARY_SOURCE, _SWEEP_BOUNDARY_SOURCE})
_OCR_VALID_START_BY = frozenset({"ocr_combat", "refined_combat", "vision_gap_sweep"})
...
_BROADCAST_MAX_ROUND_DURATION_SEC = 150.0
_OCR_FINALIZE_OVERLAP_SEC = 120.0
# 收尾 fresh full 不再一次性扫描整段录像；每个分片最多推进 90s 新内容，
# 保留有限重叠以维持 FSM/边界连续性，最后一个分片才 force_close。
_FINALIZATION_CHUNK_SEC = 90.0
_FINALIZATION_CHUNK_OVERLAP_SEC = 8.0
...
# 粗扫 asyncio 入口的并发上限；审计入口使用独立的 _refine_semaphore。
# DirectML 的 ONNX 上下文仍通过线程资源串行化，但审计只在一个候选步骤内
# 持有它，避免一整个 audit batch 长时间占用资源。
_analysis_semaphore = asyncio.Semaphore(1)
# 后台边界审计专用 asyncio 信号量（A-05）：审计与粗扫不再共享同一把
# asyncio 信号量。粗扫 kick 时通过 refine_abort 抢占审计，审计经
# cancel_check 数秒内退出并释放线程信号量；粗扫不再被整批审计阻塞数分钟，
# 覆盖游标持续推进。线程信号量仍全局唯一，保证 OCR/DirectML 不并发。
_refine_semaphore = asyncio.Semaphore(1)
# asyncio 信号量只覆盖持续分析事件循环；单次分析运行在 executor 线程中，
# 需要同一把线程信号量才能避免它绕过持续分析直接并发访问 OCR/DirectML。
_analysis_thread_semaphore = threading.Semaphore(1)
# Resource classes are explicit even on runtimes where ONNX/DirectML must stay
# serialized. The refine path acquires these only for one candidate step; it
# never holds them for the entire audit queue.
_ffmpeg_capture_semaphore = threading.Semaphore(1)
# One ONNX/DirectML context remains serialized with one-shot analysis; the
# audit-specific wrapper below releases it after each candidate step.
_onnx_inference_semaphore = _analysis_thread_semaphore
...
def _stop_idle_shared_ingest(room_id: str, reason: str) -> bool:
    ...
    try:
        ...
        return False
def _even_preview_dimension(value: int | float) -> int:
    """将预览输出尺寸向下取偶数，满足 FFmpeg/YUV 编码约束。"""
    try:
...
def _compute_preview_quality_params(data: dict | None = None) -> dict[str, Any]:
    ...
    width = _even_preview_dimension(width)
    height = _even_preview_dimension(height)
...
def _stop_live_preview_streamer(room_id: str) -> None:
    ...
    _stop_idle_shared_ingest(room_id, reason="live preview stopped")
async def _degrade_preview_offline(
    srv,
    ...
    *,
    ...
    """主播下播：停直播预览 sink 并把房间标记为 degraded，**不启动任何文件流**。

    方案 A（2026-09-11）：回看不再经后端流通道（原文件回看流 helper 已整条删除）。
    前端看到 preview_mode == 'degraded' 且有 record_output_path 时改走本地文件回看，
    因此后端只需：停（可选）录制 → 停直播预览 sink → 标注降级并广播 rooms_updated /
    preview_phase。preview_enabled 保持旧语义：仍有可回看的录制文件时保持 True
    （预览卡继续挂着，由前端切到本地文件回看），完全没有可用录制文件时置 False。
    """
    def _read_room_state():
        room = mgr.get_room(room_id)
        if room is None:
            return None
        return {
    ...
    try:
        state = await loop.run_in_executor(
            _bridge_executor, lambda: bridge.manager.call(_read_room_state)
        )
    except Exception as exc:
        _log.error("offline degraded state read failed: %s", exc)
        return False
    if state is None:
        return False
    if not state["preview_enabled"]:
        _log.debug("offline degraded skipped (preview disabled): room_id=%s", room_id)
        return False
    if stop_recording_if_active and state["is_recording"]:
        ...
        try:
            ...
            _log.warning("offline degraded stop recording failed: %s", exc)
        await asyncio.sleep(0.5)
    # 直播源已死：停掉直播预览 sink，避免残留进程反复重连（旧实现是「为文件回看让路」）。
    def _stop_live():
    ...
    try:
        ...
        _log.debug("offline degraded live preview stop failed: %s", exc)
    _mse_reconnect_state.pop(room_id, None)
    ...
    try:
        ...
        _log.error("offline degraded validate failed: %s", exc)
        valid, path, validation_err = False, "", str(exc)
    friendly = offline_message or "主播已下线"
    if not valid or not path:
        detail = validation_err or "没有可回看的录制文件"
    ...
    def _mark_degraded():
        ...
        # 有可回看的录制文件 → 保持 preview_enabled，前端据此切本地文件回看；
        # 没有 → 置 False（旧「录制文件无效」分支的语义）。
        room.preview_enabled = bool(valid and path)
        if not (valid and path):
            _clear_preview_clock(room)
        if offline_message:
            room.last_error = offline_message
        return True
    try:
        await loop.run_in_executor(
            _bridge_executor, lambda: bridge.manager.call(_mark_degraded)
        )
    except Exception as exc:
    ...
    _log.info(
        "房间已降级为本地文件回看（不启动文件流）: room_id=%s, path=%s, error=%s",
        room_id, path, friendly,
    )
    return True
...
def _clear_preview_clock(room: Any) -> None:
    """清除房间当前直播预览时钟映射。"""
    clear_preview_clock = getattr(room, "clear_preview_clock", None)
    if callable(clear_preview_clock):
        clear_preview_clock()
def _set_preview_epoch(room: Any, epoch_id: str) -> None:
    """切换预览 epoch，并同步清除旧录制轴→预览轴映射。"""
    room.preview_epoch_id = epoch_id
    _clear_preview_clock(room)
...
def _drop_open_tail_rounds(
    ...
    - 明确闭合（end_by 在 _OCR_VALID_END_BY 或赛事审计出点 broadcast_exclusion）
      的回合按 tip 距离保留：结算帧本来就贴近扫描尖端，误删会导致「有交战
      内容但 0 回合」；赛事审计通过的回合边界已由视觉模型确认，同样不得丢弃。
    """
    if not rounds:
        return []
    cleaned = list(rounds)
    ...
    if end_by in _OCR_VALID_END_BY or end_by in _BROADCAST_VALID_END_BY:
        return cleaned
...
def _is_ocr_round(round_data: dict[str, Any]) -> bool:
    """可入列候选：纯 OCR 路径产出，或收尾缺口补扫合成（来源标记可辨）。"""
    return round_data.get("boundary_source") in _LISTABLE_BOUNDARY_SOURCES
def _set_boundary_quality(round_data: dict[str, Any]) -> str:
    ...
    quality = classify_boundary_quality(
        ...
        boundary_refined=bool(round_data.get("boundary_refined")),
        start_confidence=round_data.get("start_confidence"),
        end_confidence=round_data.get("end_confidence"),
        start_delta=round_data.get("start_delta"),
        end_delta=round_data.get("end_delta"),
        source_profile=round_data.get("source_profile"),
        broadcast_audit=round_data.get("broadcast_audit"),
        broadcast_audit_reason=round_data.get("broadcast_audit_reason") or round_data.get("broadcast_excluded_reason"),
    )
    round_data["boundary_quality"] = quality
    round_data["boundary_review_required"] = quality != "precise"
    round_data["boundary_audit_version"] = 1
    round_data["boundary_quality_reason_code"] = boundary_quality_reason_code(
        confirm_status=round_data.get("confirm_status"),
        end_by=round_data.get("end_by"),
        boundary_refined=bool(round_data.get("boundary_refined")),
        start_confidence=round_data.get("start_confidence"),
        ...
        source_profile=round_data.get("source_profile"),
        broadcast_audit=round_data.get("broadcast_audit"),
        broadcast_audit_reason=round_data.get("broadcast_audit_reason")
        or round_data.get("broadcast_excluded_reason"),
    )
    if quality == "pending":
        round_data["boundary_review_reason"] = (
            f"{round_data['boundary_quality_reason_code']}: "
            f"出点证据不足: {round_data.get('end_by') or 'unknown'}"
        )
    elif quality == "coarse":
        round_data["boundary_review_reason"] = (
            f"{round_data['boundary_quality_reason_code']}: 尚未完成边界密扫"
        )
    elif quality == "invalid":
        round_data["boundary_review_reason"] = (
            f"{round_data['boundary_quality_reason_code']}: 边界证据或时间误差异常"
        )
    else:
        round_data.pop("boundary_review_reason", None)
    return quality
def _hybrid_clip_metadata(round_data: dict[str, Any]) -> dict[str, Any]:
    """Return the small evidence payload forwarded to clip_queued."""
    _set_boundary_quality(round_data)
    keys = (
        "start_by",
        "end_by",
        "boundary_source",
        ...
        "boundary_review_reason",
        "boundary_audit_version",
        "boundary_quality_reason_code",
        "start_quality",
        "end_quality",
        ...
        # 审计异常放行的候选：转发复核标记与原因，供前端明确标注“待人工复核”。
        "broadcast_review_required",
        "broadcast_audit_error",
        # 出点被视觉否决（OCR next_prep 与后续画面矛盾）：转发标记与原因，前端可据此
        # 说明"出点证据不足 / 正在按视觉证据重算"，而不是只看到一个无解释的 pending。
...
def _is_listable_ocr_round(round_data: dict[str, Any]) -> bool:
    ...
    if end <= start:
        return False
    if round_data.get("source_profile") == "broadcast":
        audit = round_data.get("broadcast_audit")
        if audit == "skipped":
            # 审计不可用（模型加载/抽帧异常）：按 R6.3 fail-safe 设计放行 OCR
            # 候选入列待人工复核，而非静默丢弃（否则审计一旦异常该窗口回合
            # 全部消失且无用户可见反馈）。仍受基本 sanity 约束，且不自动导出
            # （_is_auto_exportable_valorant_round 要求 broadcast_audit=="passed"）。
            if not round_data.get("broadcast_review_required"):
                return False
            if end - start > _BROADCAST_MAX_ROUND_DURATION_SEC:
                return False
            return str(round_data.get("start_by", "") or "") in _OCR_VALID_START_BY
        # 阶段一（A-01）：粗筛候选先行入列展示（confirm_status == "pending"）
        # 允许待复核候选即时呈现于切片列表，消除无反馈滞后感；自动导出仍受
        # _is_auto_exportable_valorant_round 严格门禁把关（必须 vision_confirmed + passed）。
        if round_data.get("confirm_status") == "pending":
            if end - start > _BROADCAST_MAX_ROUND_DURATION_SEC:
                return False
            start_by_ok = str(round_data.get("start_by", "") or "") in _OCR_VALID_START_BY
            end_by_val = str(round_data.get("end_by", "") or "")
            end_by_ok = end_by_val in _OCR_VALID_END_BY or end_by_val in _BROADCAST_VALID_END_BY
            return start_by_ok and end_by_ok
        # 赛事/二路定稿切片必须经过视觉阶段审计并通过
        if audit != "passed":
            return False
        ...
        if end - start > _BROADCAST_MAX_ROUND_DURATION_SEC:
            return False
        if str(round_data.get("end_by", "") or "") not in _BROADCAST_VALID_END_BY:
            return False
    # 异常时长守卫：过长回合不得自动导出，降级 pending 待人工复核（仍入列）
    if end - start > _VALORANT_MAX_ROUND_DURATION_SEC:
        round_data['duration_anomaly'] = True
        _log.warning(
            "OCR 回合时长异常 %.1fs > %.0fs，保留 confirm_status=%s，"
            "由 boundary_quality=invalid 表达待复核: round=%s",
            end - start,
            _VALORANT_MAX_ROUND_DURATION_SEC,
            round_data.get('confirm_status'),
    ...
    if round_data.get("source_profile") == "broadcast":
        valid_end_by = (
            _OCR_VALID_END_BY | _BROADCAST_VALID_END_BY
            if round_data.get("confirm_status") == "pending"
            else _BROADCAST_VALID_END_BY
        )
    else:
        valid_end_by = _OCR_VALID_END_BY | _BROADCAST_VALID_END_BY
    return start_by in _OCR_VALID_START_BY and end_by in valid_end_by
def _is_auto_exportable_valorant_round(round_data: dict[str, Any]) -> bool:
    ...
    if round_data.get("confirm_status") != "vision_confirmed":
        return False
    if round_data.get("duration_anomaly") or (end - start > _VALORANT_MAX_ROUND_DURATION_SEC):
        return False
    if round_data.get("source_profile") == "broadcast":
        if round_data.get("broadcast_audit") != "passed":
            return False
        if round_data.get("broadcast_review_required"):
            return False
        if end - start > _BROADCAST_MAX_ROUND_DURATION_SEC:
            return False
        if str(round_data.get("end_by", "") or "") not in _BROADCAST_VALID_END_BY:
    ...
    valid_end_by = (
        ...
        if round_data.get("source_profile") == "broadcast"
        else (_OCR_VALID_END_BY | _BROADCAST_VALID_END_BY)
    )
    return start_by in _OCR_VALID_START_BY and end_by in valid_end_by
...
def _refine_delivery_key(
    candidate: dict[str, Any],
    *,
    room_id: str = "",
    recording_id: str = "",
) -> str:
    """Build the idempotency key used by the refine delivery channel."""
    candidate_room = str(room_id or candidate.get("room_id") or "")
    candidate_recording = str(
        recording_id or candidate.get("recording_id") or ""
    )
    return ":".join(
        (candidate_room, candidate_recording, _valorant_round_key(candidate))
    )


def _finalization_job_from_state(state: dict[str, Any]) -> FinalizationJob | None:
    """从任务状态权威 payload 重建收尾 job 对象（coverage/队列以此为准）。

    worker 本地保存的 FinalizationJob 对象不会感知 ``_record_continuous_scan_coverage``
    等路径写入 state 的 coverage 账本；直接用本地对象 ``to_dict()`` 回写 sidecar
    会把 coverage 清空（``is_fully_covered`` 恒 False → 收尾无限补扫）。任何对
    job 的读取/变更都必须先从 state 重建。
    """
    payload = (state or {}).get("finalization_job")
    if not isinstance(payload, dict):
        return None
    try:
        return FinalizationJob.from_dict(payload)
    except (TypeError, ValueError):
        return None


def _persist_refine_result_queue(task_state: dict[str, Any]) -> None:
    """Persist the delivery queue when a finalization checkpoint exists.

    The live task normally drains this queue in the same event-loop turn. During
    shutdown/finalization, however, the sidecar is the recovery boundary, so
    enqueueing a result must be reflected there before the pending candidate is
    removed.
    """
    payload = task_state.get("finalization_job")
    if not isinstance(payload, dict):
        return
    try:
        job = FinalizationJob.from_dict(payload)
    except (TypeError, ValueError):
        _log.warning("精修结果队列 checkpoint 无法解析")
        return
    queue = task_state.get("refine_result_queue") or []
    job.refine_result_queue = [
        dict(item) for item in queue if isinstance(item, dict)
    ]
    job.accepted_candidates = [
        dict(item)
        for item in (task_state.get("accepted_candidates") or [])
        if isinstance(item, dict)
    ]
    job.rejected_candidates = [
        dict(item)
        for item in (task_state.get("rejected_candidates") or [])
        if isinstance(item, dict)
    ]
    runtime_state = task_state.get("ocr_runtime_state")
    if isinstance(runtime_state, dict):
        job.replace_pending_candidates(
            runtime_state.get("broadcast_pending_rounds") or []
        )
    serialized = job.to_dict()
    task_state["finalization_job"] = serialized
    if job.source_path:
        save_finalization_job(job.source_path, serialized)


def _strip_room_scoped_keys(room_id: str, *registries: Any) -> int:
    """清空所有注册表中以 ``room_id:`` 为前缀的键（dict 或 set 通用）。

    返回清除的键数。供新录制 epoch 的切片键位/精修冻结清理使用。
    """
    prefix = f"{room_id}:"
    removed = 0
    for reg in registries:
        if isinstance(reg, dict):
            for key in [k for k in reg if str(k).startswith(prefix)]:
                reg.pop(key, None)
                removed += 1
        elif isinstance(reg, (set, frozenset)):
            for key in [k for k in reg if str(k).startswith(prefix)]:
                reg.discard(key)
                removed += 1
    return removed
...
def _continuous_epoch_changed(
    prev_state: dict[str, Any] | None,
    room: Any,
    *,
    resume: bool,
) -> bool:
    """判断持续分析启动是否进入了新录制 epoch。

    - resume（崩溃恢复）延续同一录制，不算新 epoch；
    - 上一会话或当前房间任一 recording_id 缺失时，按新 epoch 处理
      （缺信息时宁可清空，也不把旧切片带进新录制）；
    - recording_id 不同即新 epoch。
    """
    if resume:
        return False
    prev_rec = str((prev_state or {}).get('recording_id') or '')
    room_rec = str(getattr(room, 'recording_id', '') or '')
    return not prev_rec or not room_rec or prev_rec != room_rec


def _prune_rejected_listed_clips(
    task_state: dict[str, Any],
    candidate: dict[str, Any],
) -> list[str]:
    """审计拒绝终态后，从权威切片快照移除该回合（覆盖所有目标房间）。

    被拒候选此前已按 pending 入列（clip_queued）；拒绝必须同步从
    ``listed_clips`` 清除，否则前端/剪映草稿会混入已被审计拒绝的旧切片。
    返回被移除的 listed_key 列表（``room:round_key``）。
    """
    if not isinstance(candidate, dict) or not isinstance(task_state, dict):
        return []
    round_key = _valorant_round_key(candidate)
    if not round_key:
        return []
    listed = task_state.get("listed_clips")
    removed: list[str] = []
    if isinstance(listed, dict):
        stale = [
            ...
            if str(listed_key) == round_key
            or str(listed_key).endswith(f":{round_key}")
        ]
        for listed_key in stale:
            listed.pop(listed_key, None)
            removed.append(str(listed_key))
    return removed


# 扫描通路终态 → durable outcome 的映射（与精修通路 `_record_audit_outcome` 同口径）
_SCAN_TERMINAL_OUTCOMES = {
    "passed": "accepted",
    "pending_no_exclusion": "manual_review",
}


def _scan_terminal_outcome(item: dict[str, Any]) -> str | None:
    """把一条扫描结果的审计结论映射成 durable outcome；非终态返回 None。"""
    audit = str(item.get("broadcast_audit") or "").strip().lower()
    if not audit or audit in {"pending_lookahead", "skipped"}:
        return None
    if audit.startswith("rejected"):
        return "rejected"
    return _SCAN_TERMINAL_OUTCOMES.get(audit)


def _rejection_reason(candidate: dict[str, Any]) -> str:
    """拒绝原因：优先审计结论，其次入点门禁，最后兜底 "rejected"。

    入点门禁通过时会留下 `broadcast_start_gate="ok"`，直接取它会让前端显示
    「已拒绝(ok)」（L3 实测），无法分辨是回放/非游戏还是入点不稳导致的拒绝。
    """
    audit = str(candidate.get("broadcast_audit") or "").strip()
    if audit.startswith("rejected"):
        return audit
    gate = str(candidate.get("broadcast_start_gate") or "").strip()
    if gate and gate.lower() != "ok":
        return gate
    return audit or gate or "rejected"


def _terminal_round_keys(task_state: dict[str, Any]) -> set[str]:
    """已有归属的 round_key：durable 终态 + 拒绝 tombstone + 待审计队列 + 待交付队列。"""
    known: set[str] = set()
    for bucket in ("accepted_candidates", "rejected_candidates"):
        for item in task_state.get(bucket) or []:
            if isinstance(item, dict):
                known.add(_valorant_round_key(item))
    known |= {str(key) for key in (task_state.get("rejected_round_keys") or {})}
    pending_queue = (task_state.get("ocr_runtime_state") or {}).get("broadcast_pending_rounds") or []
    for item in pending_queue:
        if isinstance(item, dict):
            known.add(_valorant_round_key(item))
    for entry in task_state.get("refine_result_queue") or []:
...
def _listed_items_without_terminal(task_state: dict[str, Any]) -> list[dict[str, Any]]:
    """列在权威切片快照、却既无终态也不在待审计队列的回合（返回其 listed 条目）。

    现场：20:45:20 收尾完成判定那一刻待审计队列刚清零（round-000135 在 20:45:17
    才出结论），completed 提前成立，135 的归属与结论就此错位——导出时被判
    「不在权威集合」而不是「出点未定稿」。
    """
    listed = task_state.get("listed_clips") or {}
    if not isinstance(listed, dict) or not listed:
        return []
    known = _terminal_round_keys(task_state)
    out: list[dict[str, Any]] = []
    for key, item in listed.items():
        if not isinstance(item, dict):
            continue
        round_key = str(item.get("round_key") or "") or str(key).split(":", 1)[-1]
        if not round_key or round_key in known:
            continue
        out.append(dict(item))
    return out


def _project_scan_audit_terminals(task_state: dict[str, Any], highlights: Any) -> int:
    """把扫描通路产出的审计终态补进 durable 投影（幂等，按 round_key）。

    扫描通路的结论此前只进 ``listed_clips``：``audit_terminal_total`` / accepted /
    rejected 账本与收尾 sidecar 都看不到它，于是"任务态一 pop、权威回落旧 sidecar"
    就把已定稿切片改回 pending（2026-09-11 20:45 的 round-000105）。
    这里保持与 ``_record_audit_outcome`` 同一映射，不改变任何门禁判据。
    """
    if not isinstance(task_state, dict) or not highlights:
        return 0
    known: set[str] = set()
    for bucket in ("accepted_candidates", "rejected_candidates"):
        for item in task_state.get(bucket) or []:
    ...
    for entry in task_state.get("refine_result_queue") or []:
        if isinstance(entry, dict):
            ...
            if isinstance(candidate, dict):
                known.add(_valorant_round_key(candidate))
    recorded = 0
    for item in highlights:
        if not isinstance(item, dict):
            continue
        outcome = _scan_terminal_outcome(item)
        if not outcome:
            continue
        key = _valorant_round_key(item)
        if not key or key in known:
            continue
        known.add(key)
        if outcome == "rejected":
            task_state["audit_terminal_total"] = int(task_state.get("audit_terminal_total") or 0) + 1
            task_state["audit_rejected_count"] = int(task_state.get("audit_rejected_count") or 0) + 1
            task_state["last_audit_rejection"] = {
                "start": float(item.get("start") or 0.0),
                "end": float(item.get("end") or 0.0),
                "reason": str(item.get("broadcast_audit") or "rejected"),
            }
            _record_terminal_candidate_state(task_state, item, "rejected")
            _prune_rejected_listed_clips(task_state, item)
        else:
            # 扫描通路的结果**已经**入列（本次调用就发生在入列路径上），
            # 因此同时计入 delivered，delivery_gap 不会因补投影而虚增。
            task_state["audit_terminal_total"] = int(task_state.get("audit_terminal_total") or 0) + 1
            task_state["audit_delivered_total"] = int(task_state.get("audit_delivered_total") or 0) + 1
            if outcome == "accepted":
                task_state["audit_accepted_count"] = int(task_state.get("audit_accepted_count") or 0) + 1
            else:
                task_state["audit_manual_review_count"] = int(task_state.get("audit_manual_review_count") or 0) + 1
            _record_terminal_candidate_state(task_state, item, outcome)
        recorded += 1
    if recorded:
        _log.info(
            "扫描通路终态已补投影: room_id=%s, 新增 %d 条, terminal_total=%s, accepted=%s, rejected=%s",
            task_state.get("room_id") or "", recorded,
            task_state.get("audit_terminal_total"),
            task_state.get("audit_accepted_count"),
            task_state.get("audit_rejected_count"),
        )
    return recorded


def _project_scan_audit_terminals_for_room(room_id: str, highlights: Any) -> int:
    """按房间找任务态并补投影（任务态不存在时为空操作）。"""
    if not room_id or not highlights:
        return 0
    with _analysis_jobs_lock:
        task_state = _continuous_tasks.get(room_id)
    if not isinstance(task_state, dict):
        return 0
    return _project_scan_audit_terminals(task_state, highlights)


# ── L2：定稿后组内择一（同一真实回合被多条候选认领时只保留一条）───────────
# 现场（2026-09-11 20:45）：round-000123 与 round-000135 是同一段真实回合的两个候选
#（123 的区间跨了回合边界，见 L1；修好 L1 前它们的定稿区间重叠 68.75s）。
# 归组只看「出点已定稿的 broadcast 切片」：未定稿/被拒的仍由各自门禁处理，
# 不在这里"被合并"（071 END_NOT_FINAL、123 跨回合都不该由 L2 收口）。
_DEDUPE_OVERLAP_MIN = 0.2
_DEDUPE_MIN_SPAN_SEC = 5.0


def _round_dedupe_key(item: dict[str, Any]) -> str:
    return str(item.get("round_key") or "") or str(item.get("clip_id") or "")


def _round_span_sec(item: dict[str, Any]) -> tuple[float, float] | None:
    try:
        start = float(item.get("start"))
        end = float(item.get("end"))
    except (TypeError, ValueError):
        return None
    if end - start < _DEDUPE_MIN_SPAN_SEC:
        return None
    return start, end


def _round_overlap_ratio(a: tuple[float, float], b: tuple[float, float]) -> float:
    """重叠度 = 交集 / 较短区间（包含关系即 1.0）。

    比 IoU 更适合"一小条落在长条里"（同一回合的截断块）这类形态。
    """
...
def _is_finalized_broadcast_round(item: dict[str, Any]) -> bool:
    """出点已定稿的赛事切片（与 `_broadcast_gate_passed` 同一判据，仅用于归组）。"""
    if not isinstance(item, dict):
        return False
    if str(item.get("source_profile") or "").strip().lower() != "broadcast":
        return False
    if str(item.get("confirm_status") or "").strip().lower() in ("rejected", "refining"):
        return False
    if str(item.get("broadcast_audit") or "").strip().lower() != "passed":
        return False
    if str(item.get("end_quality") or "").strip().lower() != "precise":
        return False
    if bool(item.get("end_review_required")) or bool(item.get("duration_anomaly")):
        return False
    return str(item.get("end_by") or "") in _BROADCAST_VALID_END_BY


def _round_dedupe_rank(item: dict[str, Any]) -> tuple:
    """择一优先级（升序，越小越优先）：起点证据强 → 无内部边界 → 区间更长 → 起点更早 → key。"""
    has_start_evidence = bool(
        item.get("start_delta") is not None
        or str(item.get("start_quality") or "").strip().lower() == "precise"
    )
    span = _round_span_sec(item)
    length = (span[1] - span[0]) if span else 0.0
    start = span[0] if span else float("inf")
    return (
        0 if has_start_evidence else 1,
        0 if not item.get("interior_boundary_resume_sec") else 1,
        -length,
        start,
        _round_dedupe_key(item),
    )


def _dedupe_overlapping_rounds(
    rounds: list[dict[str, Any]],
    *,
    overlap_min: float = _DEDUPE_OVERLAP_MIN,
) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    """L2：把"同一真实回合被多条候选认领"的定稿切片归组择一。

    返回 ``(保留列表, 被并列表)``；被并项是**副本**，带 ``merged_into`` /
    ``duplicate_round`` / ``merge_reason``，由调用方标记权威快照并广播——不静默消失。
    归组为连通分量（允许传递：A∩B、B∩C 同组，只留一条）。
    """
    ordered = [item for item in rounds if isinstance(item, dict)]
    participants = [
        (idx, item)
        for idx, item in enumerate(ordered)
        if _is_finalized_broadcast_round(item) and _round_span_sec(item) is not None
    ]
    if len(participants) < 2:
        return ordered, []
    parent = {idx: idx for idx, _ in participants}
    ...
    for pos, (idx, item) in enumerate(participants):
        room_id = str(item.get("room_id") or "")
        span = _round_span_sec(item)
        for other_idx, other in participants[pos + 1:]:
            if str(other.get("room_id") or "") != room_id:
                continue
            other_span = _round_span_sec(other)
            if span is None or other_span is None:
                continue
            if _round_overlap_ratio(span, other_span) >= overlap_min:
                root_a, root_b = _find(idx), _find(other_idx)
                if root_a != root_b:
                    parent[root_b] = root_a
    groups: dict[int, list[int]] = {}
    for idx, _ in participants:
        groups.setdefault(_find(idx), []).append(idx)
    merged_idx: dict[int, str] = {}
    for members in groups.values():
        if len(members) < 2:
            continue
        winner = min(members, key=lambda i: _round_dedupe_rank(ordered[i]))
        winner_key = _round_dedupe_key(ordered[winner])
        for idx in members:
            if idx != winner:
                merged_idx[idx] = winner_key
    if not merged_idx:
        return ordered, []
    kept = [item for idx, item in enumerate(ordered) if idx not in merged_idx]
    merged: list[dict[str, Any]] = []
    for idx in sorted(merged_idx):
        loser = dict(ordered[idx])
        loser["merged_into"] = merged_idx[idx]
        loser["duplicate_round"] = True
        loser["merge_reason"] = "duplicate_round"
        merged.append(loser)
    return kept, merged


def _mark_merged_rounds(
    room_id: str,
    merged: list[dict[str, Any]],
    *,
    broadcast: Any | None = None,
) -> int:
    """被并回合标进权威快照 + durable 拒绝账本，并广播（原因码 duplicate_round）。"""
    if not merged:
        return 0
    with _analysis_jobs_lock:
        task_state = _continuous_tasks.get(room_id)
    snapshot = (task_state or {}).get("listed_clips") if isinstance(task_state, dict) else None
    payload_base = {
        "confirm_status": "rejected",
        "broadcast_audit": "rejected_duplicate_round",
        "broadcast_audit_reason": "duplicate_round",
    }
    marked = 0
    for item in merged:
        key = _valorant_round_key(item)
        if not key:
            continue
        payload = {**payload_base, "merged_into": item.get("merged_into"), "duplicate_round": True}
        if isinstance(snapshot, dict):
            for listed in snapshot.values():
                if isinstance(listed, dict) and _valorant_round_key(listed) == key:
                    listed.update(payload)
        if isinstance(task_state, dict):
            _record_terminal_candidate_state(task_state, {**item, **payload}, "rejected")
            _persist_refine_result_queue(task_state)
        if broadcast is not None:
            broadcast({
                ...
                "data": {
                    ...
                    "start": round(float(item.get("start") or 0.0), 1),
                    "end": round(float(item.get("end") or 0.0), 1),
                },
            })
        marked += 1
    if marked:
        _log.info(
            "同一回合去重（L2）: room_id=%s, 并入 %d 条: %s",
            room_id,
            marked,
            ", ".join(f"{_valorant_round_key(i)}→{i.get('merged_into')}" for i in merged),
        )
    return marked


def _record_terminal_candidate_state(
    task_state: dict[str, Any],
    candidate: dict[str, Any],
    outcome: str,
) -> None:
    """Keep in-memory + sidecar terminal projections consistent.

    ``accepted_candidates``/``rejected_candidates`` are the durable authoritative
    terminal state used after crash/restart.  ``rejected_round_keys`` is the fast
    tombstone consumed by the merge projection so a rejected candidate cannot be
    resurrected by a stale all_highlights or sidecar highlights entry.
    """
    if not isinstance(candidate, dict):
        return
    normalized = str(outcome or "").strip().lower()
    key = _valorant_round_key(candidate)
    if normalized in {"accepted", "manual_review"}:
        target = task_state.setdefault("accepted_candidates", [])
    elif normalized == "rejected":
        target = task_state.setdefault("rejected_candidates", [])
        task_state.setdefault("rejected_round_keys", {})[key] = {
            "start": candidate.get("start"),
            "end": candidate.get("end"),
            # 拒绝原因优先取审计结论：start_gate 通过（"ok"）时也会被记为拒绝原因，
            # 前端就会看到「已拒绝(ok)」这种没有信息量的文案（L3 实测）。
    ...
    for index, item in enumerate(target):
        if isinstance(item, dict) and _valorant_round_key(item) == key:
    ...
    if normalized == "rejected":
        task_state["rejected_round_keys"][key] = {
            "start": candidate.get("start"),
            "end": candidate.get("end"),
            "reason": _rejection_reason(candidate),
            "recorded_at": time.time(),
...
def _resume_refine_result_queue(resume_job: Any | None) -> list[dict[str, Any]]:
    """Rebuild the live refine delivery queue after restart.

    ``refine_result_queue`` only stores results that have not yet been merged.
    After a crash, accepted results may already have been acked from that queue,
    so the durable ``accepted_candidates`` projection must be re-enqueued for the
    main loop to list them again.
    """
    if resume_job is None:
        return []
    queue = [
        dict(item)
        for item in resume_job.refine_result_queue
        if isinstance(item, dict)
    ]
    seen = {
        str(item.get("delivery_key") or "")
        for item in queue
        if isinstance(item, dict) and item.get("delivery_key")
    }
    for candidate in resume_job.accepted_candidates:
        if not isinstance(candidate, dict):
            continue
        key = _refine_delivery_key(
            candidate,
            room_id=str(resume_job.room_id or ""),
            recording_id=str(resume_job.recording_id or ""),
        )
...
def _enqueue_refine_result(
    ...
    *,
    ...
    """Enqueue a terminal audit result before it leaves the pending queue.

    This is the P0 delivery boundary. It is intentionally independent from the
    worker's ``scan_result_container``: a slow/failed later candidate cannot
    overwrite an already accepted result.
    """
    if not isinstance(candidate, dict):
        return False
    normalized_outcome = str(outcome or "accepted").strip().lower()
    if normalized_outcome not in {"accepted", "manual_review"}:
        return False
    key = _refine_delivery_key(
        candidate,
        room_id=room_id or str(task_state.get("room_id") or ""),
        recording_id=recording_id or str(task_state.get("recording_id") or ""),
    )
    with _analysis_jobs_lock:
        queue = task_state.setdefault("refine_result_queue", [])
        if any(
            isinstance(item, dict)
            and str(item.get("delivery_key") or "") == key
            for item in queue
        ):
            return False
        delivered = task_state.setdefault("refine_delivered_keys", {})
        if key in delivered:
            return False
        queue.append({
            "delivery_key": key,
        ...
        task_state["refine_result_queue_depth"] = len(queue)
        task_state["audit_terminal_total"] = int(
            task_state.get("audit_terminal_total") or 0
        ) + 1
        if normalized_outcome == "accepted":
            task_state["audit_accepted_count"] = int(
                task_state.get("audit_accepted_count") or 0
            ) + 1
        else:
            task_state["audit_manual_review_count"] = int(
                task_state.get("audit_manual_review_count") or 0
            ) + 1
        accepted_total = int(task_state.get("audit_accepted_count") or 0)
        delivered_total = int(task_state.get("audit_delivered_total") or 0)
        task_state["audit_delivery_gap"] = max(0, accepted_total - delivered_total)
        _record_terminal_candidate_state(task_state, candidate, normalized_outcome)
    _log.info(
        "精修候选终态入可靠队列: room_id=%s, recording_id=%s, round_key=%s, "
        "candidate_state_before=%s, audit_outcome=%s, delivery_state=queued, "
        "listed_state_after=pending_delivery, elapsed_sec=%.3f",
        room_id or task_state.get("room_id", ""),
        recording_id or task_state.get("recording_id", ""),
        _valorant_round_key(candidate),
        candidate.get("broadcast_audit") or candidate.get("confirm_status") or "unknown",
        normalized_outcome,
        max(
            0.0,
            time.time() - float(task_state.get("refine_started_at") or time.time()),
        ),
    )
    return True


def _peek_refine_results(task_state: dict[str, Any]) -> list[dict[str, Any]]:
    """Return a copy of queued terminal results without acknowledging them."""
    with _analysis_jobs_lock:
        return [
            dict(item)
            for item in (task_state.get("refine_result_queue") or [])
            if isinstance(item, dict) and isinstance(item.get("candidate"), dict)
        ]


def _ack_refine_results(
    task_state: dict[str, Any],
    delivery_keys: list[str],
) -> int:
    """Acknowledge results only after the main loop has merged/listed them."""
    ...
    with _analysis_jobs_lock:
        ...
        for item in queue:
            if (
                isinstance(item, dict)
                and str(item.get("delivery_key") or "") in keys
            ):
                removed += 1
                continue
            if isinstance(item, dict):
                kept.append(item)
        task_state["refine_result_queue"] = kept
        task_state["refine_result_queue_depth"] = len(kept)
        delivered = task_state.setdefault("refine_delivered_keys", {})
        for key in keys:
            delivered[key] = None
        if len(delivered) > 20000:
            for old_key in list(delivered)[:10000]:
                delivered.pop(old_key, None)
        task_state["audit_delivered_total"] = int(
            task_state.get("audit_delivered_total") or 0
        ) + removed
        accepted_total = int(task_state.get("audit_accepted_count") or 0)
        task_state["audit_delivery_gap"] = max(
            0, accepted_total - int(task_state.get("audit_delivered_total") or 0)
        )
        _persist_refine_result_queue(task_state)
    if removed:
        _log.info(
            "精修候选已由主循环消费: room_id=%s, delivered=%d, "
            "delivery_state=delivered, listed_state_after=merged, elapsed_sec=%.3f",
            task_state.get("room_id", ""),
            removed,
            max(
                0.0,
                time.time() - float(task_state.get("refine_started_at") or time.time()),
...
def _should_broadcast_clip_list_update(
    ...
    *,
    ...
    if deleted_keys and listed_key in deleted_keys:
        return "skip"
    # 精修冻结键为 ``room:round_key`` 作用域（listed_key 即该形式）；
    # 兼容旧数据中可能残留的裸 round_key 冻结。
    if listed_key in refined_keys or round_key in refined_keys:
        return "skip"
...
def _apply_scan_timeout_backoff(state: dict[str, Any]) -> None:
    """超时后记录失败计数；未覆盖窗口由拆分重试处理。"""
    failures = int(state.get("consecutive_scan_timeouts") or 0) + 1
    state["consecutive_scan_timeouts"] = failures
def _schedule_scan_timeout_retry(
    state: dict[str, Any],
    scan_range: tuple[float, float] | list[float],
    error: object,
) -> tuple[float, float] | None:
    """为超时窗口安排更小的重试，绝不通过推进游标跳过内容。
    每次超时将当前区间二分，重试任务只消费前半段；成功后游标自然推进，
    下一次正常预算会从该游标继续覆盖后半段。窗口小于最小值时保留原区间，
    由失败退避限制重试频率，但仍不能静默标记为已覆盖。
    """
    if not isinstance(scan_range, (list, tuple)) or len(scan_range) < 2:
        return None
    try:
        start = max(0.0, float(scan_range[0]))
        end = float(scan_range[1])
    except (TypeError, ValueError):
        return None
    if end <= start:
        return None
    _apply_scan_timeout_backoff(state)
    state.setdefault("last_analyzed", 0.0)
    state["last_scan_error"] = str(error or "scan timeout")
    state["last_scan_error_at"] = time.time()
    span = end - start
    if span > _SCAN_MIN_RETRY_WINDOW_SEC:
        end = start + max(_SCAN_MIN_RETRY_WINDOW_SEC, span / 2.0)
    retry_range = (round(start, 3), round(end, 3))
    state["retry_scan_range"] = retry_range
    state["retry_scan_depth"] = int(state.get("retry_scan_depth", 0) or 0) + 1
    state["analysis_stage"] = "超时拆分重试"
    return retry_range
...
def _note_successful_scan_after_degrade(state: dict[str, Any]) -> None:
    """成功消费一次扫描后清除超时/拆分重试状态。"""
    state["consecutive_scan_timeouts"] = 0
    state.pop("retry_scan_range", None)
    state.pop("retry_scan_depth", None)
def _continuous_listed_clip_snapshot(task: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    return []
def _continuous_coverage_snapshot(task: dict[str, Any]) -> list[list[float]]:
    """Return the successful scan coverage without exposing mutable state."""
    ranges = task.get("coverage_ranges") or []
    ...
    return [list(item) for item in merge_ranges((item[0], item[1]) for item in result)]
def _refresh_audit_queue_state(task_state: dict[str, Any]) -> None:
    """刷新审计队列深度与交付差额状态。"""
    ocr_state = task_state.get("ocr_runtime_state") or {}
    pending = ocr_state.get("broadcast_pending_rounds") or []
    task_state["pending_queue_depth"] = len(pending)
    accepted = int(task_state.get("audit_accepted_count") or 0)
    delivered = int(task_state.get("audit_delivered_total") or 0)
    task_state["audit_delivery_gap"] = max(0, accepted - delivered)


def _record_continuous_scan_coverage(
    state: dict[str, Any],
    scan_range: object,
    *,
    final_duration: float | None = None,
) -> None:
    """Record a successful window without advancing coverage on failures."""
    if not isinstance(scan_range, (list, tuple)) or len(scan_range) < 2:
        return
    try:
    ...
    if final_duration is not None:
        try:
            target = max(0.0, float(final_duration))
        except (TypeError, ValueError):
            target = 0.0
        state["coverage_complete"] = bool(
            target <= 0.0
            or not uncovered_ranges_for_state(state, target)
        )
    job_payload = state.get("finalization_job")
    if isinstance(job_payload, dict):
        try:
            job = FinalizationJob.from_dict(job_payload)
            job.add_coverage(start, end)
            state["finalization_job"] = job.to_dict()
            if job.source_path:
                save_finalization_job(job.source_path, state["finalization_job"])
        except (TypeError, ValueError) as exc:
            _log.warning("更新收尾 coverage 失败: %s", exc)
def uncovered_ranges_for_state(
    state: dict[str, Any],
    final_duration: float,
) -> list[tuple[float, float]]:
    """Return uncovered portions of a state ledger for status and gates."""
    return uncovered_ranges(
        ((item[0], item[1]) for item in _continuous_coverage_snapshot(state)),
        0.0,
        max(0.0, float(final_duration)),
        epsilon=2.0,
    )
def _lag_percentile(samples: list[float], percentile: float) -> float:
    """Return a deterministic nearest-rank percentile for lag telemetry."""
    if not samples:
        return 0.0
    ordered = sorted(max(0.0, float(item)) for item in samples)
    index = min(
        len(ordered) - 1,
        max(0, int(round((len(ordered) - 1) * float(percentile)))),
    )
...
def _build_continuous_status_payload(
    ...
    *,
    ...
    # 保留有限 lag 窗口，让 UI/日志能区分偶发尖峰和持续追不上；
    # 采样节流，避免状态广播本身产生大量写入。
    lag_sec = max(0.0, rec_dur - analyzed) if rec_dur else 0.0
    lag_history_raw = task.get("analysis_lag_history")
    lag_history = (
        [max(0.0, float(item)) for item in lag_history_raw if isinstance(item, (int, float))]
        if isinstance(lag_history_raw, list)
        else []
    )
    now_mono = time.monotonic()
    last_lag_sample = float(task.get("_last_lag_sample_mono", 0.0) or 0.0)
    if now_mono - last_lag_sample >= 1.0:
        lag_history = (lag_history + [lag_sec])[-120:]
        task["analysis_lag_history"] = lag_history
        task["_last_lag_sample_mono"] = now_mono
    runtime_state = task.get("ocr_runtime_state")
    broadcast_classifier = (
        runtime_state.get("broadcast_classifier")
        if isinstance(runtime_state, dict)
        else None
    )
    model_telemetry = getattr(broadcast_classifier, "telemetry", {})
    if not isinstance(model_telemetry, dict):
        model_telemetry = {}
    try:
        sample_interval = max(0.1, float(task.get("ocr_sample_interval", 1.0) or 1.0))
    except (TypeError, ValueError):
        sample_interval = 1.0
    sample_fps = round(1.0 / sample_interval, 2)
    provider = task.get("provider") or model_telemetry.get("provider")
    provider_warning = task.get("provider_warning") or model_telemetry.get("provider_warning")
    # pending_rounds 已包含尚未完成 broadcast audit 的候选，不能再把
    # audit_queue_depth 直接相加，否则 UI 会把同一个候选计数两次。
    pending_review_count = max(
        pending,
        int(task.get("audit_queue_depth", 0) or 0),
        0,
    )
    delivery_gap = int(task.get("audit_delivery_gap", 0) or 0)
    if delivery_gap:
        last_gap_log = int(task.get("_last_audit_gap_log", -1) or -1)
        if last_gap_log != delivery_gap:
            refine_queue_depth = int(task.get("refine_result_queue_depth", 0) or 0)
            terminal_gap = bool(
                refine_queue_depth <= 0
                and not task.get("scan_running")
                and not task.get("refine_running")
                and (
                    task.get("completed")
                    or task.get("cancelled")
                    or task.get("finalization_state") == "completed"
                )
            )
            log_gap = _log.error if terminal_gap else _log.warning
            log_gap(
                "%s: room_id=%s, audit_terminal_total=%s, "
                "audit_accepted_count=%s, audit_delivered_total=%s, audit_delivery_gap=%s, "
                "pending_queue_depth=%s, refine_result_queue_depth=%s",
                (
                    "持续分析审计交付对账失败"
                    if terminal_gap
                    else "持续分析审计结果待主循环消费"
                ...
                room_id,
                task.get("audit_terminal_total", 0),
                task.get("audit_accepted_count", 0),
                task.get("audit_delivered_total", 0),
                delivery_gap,
                task.get("audit_queue_depth", 0),
                refine_queue_depth,
            )
            task["_last_audit_gap_log"] = delivery_gap
    if task.get("error") or task.get("last_scan_error"):
        quality_status = "error"
    elif pending_review_count > 0 or task.get("audit_rejected_count", 0):
        quality_status = "review"
    elif provider == "CPUExecutionProvider":
        quality_status = "degraded"
    else:
        quality_status = "normal"
    # stopping 阶段单独保留同一指标，便于识别收尾二次飙升。
    ...
    payload: dict[str, Any] = {
        ...
        "planner": task.get("planner", "unknown"),
        "planned_new_media_sec": task.get("planned_new_media_sec"),
        "backlog_sec": task.get("backlog_sec", lag_sec),
        "requested_valorant_profile": task.get("requested_valorant_profile"),
        "profile_reason": task.get("profile_reason"),
        "profile_mismatch_warning": bool(task.get("profile_mismatch_warning", False)),
        "profile_warning_message": task.get("profile_warning_message", ""),
        "scan_elapsed_sec": (
            round(time.monotonic() - task.get("_scan_start_mono", time.monotonic()), 1)
            if task.get("scan_running")
            else 0
        ),
        "scan_wall_sec": task.get("last_scan_wall_sec"),
        "scan_throughput": task.get("last_scan_throughput"),
        "gross_scanned_sec": task.get("gross_scanned_sec"),
        "newly_covered_sec": task.get("newly_covered_sec"),
        "gross_scan_throughput": task.get("gross_scan_throughput"),
        "net_coverage_throughput": task.get("net_coverage_throughput"),
        "net_coverage_throughput_history": list(
            task.get("net_coverage_throughput_history") or []
        ),
        "lag_delta_sec": task.get("lag_delta_sec"),
        "lag_slope": task.get("lag_slope"),
        "audit_queue_depth": task.get("audit_queue_depth", 0),
        "audit_accepted_count": task.get("audit_accepted_count", 0),
        "audit_rejected_count": task.get("audit_rejected_count", 0),
        "audit_manual_review_count": task.get("audit_manual_review_count", 0),
        "audit_terminal_total": task.get("audit_terminal_total", 0),
        "audit_delivered_total": task.get("audit_delivered_total", 0),
        "audit_delivery_gap": task.get("audit_delivery_gap", 0),
        "pending_queue_depth": task.get("audit_queue_depth", 0),
        "refine_result_queue_depth": task.get("refine_result_queue_depth", 0),
        "finalization_pending_jobs": int(
            bool(
                task.get("finalization_job_id")
                and not task.get("completed")
            )
        ),
        "last_audit_rejection": task.get("last_audit_rejection"),
        "backlog_mode": task.get("backlog_mode", "realtime"),
        "scan_running": bool(task.get("scan_running", False)),
        "valorant_profile": task.get("valorant_profile"),
        "finalizing": finalizing,
        "completed": bool(task.get("completed", False)),
        "status": task.get("status", "running"),
        "analysis_lag_sec": lag_sec,
        "analysis_lag_median_sec": round(_lag_percentile(lag_history, 0.50), 2),
        "analysis_lag_p90_sec": round(_lag_percentile(lag_history, 0.90), 2),
        "analysis_lag_max_sec": round(max(lag_history, default=lag_sec), 2),
        "analysis_lag_sample_count": len(lag_history),
        "analysis_backlog_sec": max(
            0.0,
            float(task.get("analysis_backlog_sec", lag_sec) or lag_sec),
        ),
        "realtime_fast_mode": bool(task.get("realtime_fast_mode", False)),
        "scan_cycle_sec": float(task.get("scan_cycle_sec", 0.0) or 0.0),
        "last_scan_wall_sec": float(task.get("last_scan_wall_sec", 0.0) or 0.0),
        "scan_throughput_history": [
            float(item)
            for item in (task.get("scan_throughput_history") or [])
            if isinstance(item, (int, float))
        ],
        "sample_fps": sample_fps,
        "model_version": task.get("model_version") or model_telemetry.get("model_version"),
        "provider": provider,
        "provider_warning": provider_warning,
        "last_model_inference_frames": int(
            model_telemetry.get("last_model_inference_frames", 0) or 0
        ),
        "model_inference_frames_total": int(
            model_telemetry.get("model_inference_frames_total", 0) or 0
        ),
        "model_infer_fps": float(model_telemetry.get("model_infer_fps", 0.0) or 0.0),
        "model_infer_ms_p50": float(model_telemetry.get("model_infer_ms_p50", 0.0) or 0.0),
        "model_infer_ms_p90": float(model_telemetry.get("model_infer_ms_p90", 0.0) or 0.0),
        "pending_review_count": pending_review_count,
        "analysis_quality_status": quality_status,
        "analysis_lag_phase": resolved_phase,
        ...
        "coverage_ranges": _continuous_coverage_snapshot(task),
        "coverage_uncovered_ranges": [
            list(item)
            for item in uncovered_ranges_for_state(task, rec_dur)
        ],
        "coverage_complete": bool(task.get("coverage_complete", False)),
        "finalization_job_id": task.get("finalization_job_id"),
        "finalization_state": (
            "completed"
            if task.get("completed")
                ...
                if finalizing
                    ...
                    "checkpoint_saved"
                    if task.get("finalization_job_id")
                    else None
                )
            )
        ),
        "finalization_full_rescan": bool(task.get("finalization_full_rescan", False)),
        "finalization_full_rescan_started": bool(
            task.get("finalization_full_rescan_started", False)
        ),
    }
    # 纯 OCR/尚未启动视觉审计时，不要向前端发一组全是 None/0 的模型字段；
    # broadcast classifier 真正加载后再提供完整遥测，避免误显示“模型已运行”。
    if not (
        model_telemetry.get("model_version")
        or model_telemetry.get("provider")
        or model_telemetry.get("model_inference_frames_total")
    ):
        for _key in (
            "model_version",
            "provider",
            "provider_warning",
            "last_model_inference_frames",
            "model_inference_frames_total",
            "model_infer_fps",
            "model_infer_ms_p50",
            "model_infer_ms_p90",
        ):
    ...
    for new in window_rounds:
        ...
        for old in existing:
            ...
            if new_hybrid or not old_hybrid:
                ...
                # 但对于超大回合切分出来的子块（split_from_oversize），绝不能反向拉伸回父回合起点。
                if (
                    old_hybrid
                    and not new_item.get("split_from_oversize")
                    and old_start < new_start - 3.0
                    and old_end - old_start >= 10.0
                ):
...
def _finalize_scan_timeout(
    ...
    *,
    ...
    broadcast（赛事/二路）模式实测吞吐仅 ~1.1x 实时，需按每分钟录像 60s 预算，
    防止 16 分钟全量扫描在 589s 就被打断。
    """
    ...
    try:
        ...
        attempt_n = 1
    if is_broadcast:
        # 每分钟录像 60s 预算 + 240s 基线；重试加 180s；上限 2400s
        base = int(dur / 60.0 * 60.0 + 240.0) + (attempt_n - 1) * 180
        return int(min(2400, max(360, base)))
...
def _consume_broadcast_audit_outcome(
    ...
    *,
    ...
    """Consume one audit outcome with durable-before-remove ordering."""
    if candidate_index < 0 or candidate_index >= len(pending):
        return False
    status = str(getattr(outcome, 'status', 'pending') or 'pending').lower()
    candidate_raw = getattr(outcome, 'candidate', pending[candidate_index])
    candidate = (
        dict(candidate_raw)
        if isinstance(candidate_raw, dict)
        else dict(pending[candidate_index])
    )
    reason = str(getattr(outcome, 'reason', 'unknown') or 'unknown')
    if status in {'accepted', 'manual_review'}:
        # Delivery is the commit point.  It must happen before pending.pop().
        queued = _enqueue_refine_result(
            task_state,
            candidate,
            outcome=status,
        )
        ...
        runtime_state = task_state.get("ocr_runtime_state")
        if isinstance(runtime_state, dict):
            runtime_state["broadcast_pending_rounds"] = list(pending)
            _persist_refine_result_queue(task_state)
        if queued:
            produced_audited.append(candidate)
        return True
    if status == 'rejected':
        pending.pop(candidate_index)
        task_state['audit_terminal_total'] = int(
        ...
        task_state['audit_rejected_count'] = int(
            task_state.get('audit_rejected_count') or 0
        ) + 1
        task_state['last_audit_rejection'] = {
        ...
        _record_terminal_candidate_state(task_state, candidate, status)
        # 拒绝终态必须同步从权威切片快照移除并广播，否则已入列的 pending
        # 切片会残留到前端与剪映草稿（旧切片混入根因之一）
        removed_listed = _prune_rejected_listed_clips(task_state, candidate)
        for listed_key in removed_listed:
            listed_room = str(listed_key).split(':', 1)[0]
            if broadcast is not None:
                broadcast({
                    'type': 'clip_confirm_status',
                    'data': {
                        ...
                        'round_key': _valorant_round_key(candidate),
                        'confirm_status': 'rejected',
                        'start': round(float(candidate.get('start') or 0.0), 1),
                        'end': round(float(candidate.get('end') or 0.0), 1),
                        'reason': reason,
                    },
                })
        _log.info(
            "精修候选终态: room_id=%s, recording_id=%s, round_key=%s, "
            "candidate_state_before=%s, audit_outcome=rejected, delivery_state=not_required, "
            "listed_state_after=%s, elapsed_sec=%.3f",
            task_state.get('room_id', ''),
            task_state.get('recording_id', ''),
            _valorant_round_key(candidate),
            candidate.get('broadcast_audit') or 'unknown',
            'not_listed' if not removed_listed else f'removed:{len(removed_listed)}',
            max(0.0, time.time() - float(task_state.get('refine_started_at') or time.time())),
        )
        return True

    pending[candidate_index].update(candidate)
    pending[candidate_index]['_last_audit_dur'] = float(current_duration)
    return False


def _consume_broadcast_audit_outcome_batch(
    pending: list[dict[str, Any]],
    candidate_index: int,
    outcomes: list[Any],
    produced_audited: list[dict[str, Any]],
    ...
    *,
    ...
    """批量消费单个候选产生的全部审计 outcome（超长分裂感知）。

    ``audit_broadcast_rounds_with_outcomes`` 对超长候选分裂后会一次返回多个
    outcome（每个子候选一个）。本函数：
      - accepted/manual_review：追加到可靠结果队列与 produced_audited；
      - rejected：计入拒绝统计；
      - 非终态（pending_lookahead）：以自身 start 为主键写回 pending，等待下
        一审计周期续扫（audit_cache 保证不重复抽帧）；
      - 全部为终态时弹出原 pending 槽位。
    返回 True 表示候选已完全定稿（可推进审计配额），False 表示仍有非终态
    子候选待后视窗口，本周期应暂停。
    """
    if candidate_index < 0 or candidate_index >= len(pending):
        return False
    original = pending[candidate_index]
    slot_open = True
    rejected_count = 0
    terminal_all = True
    for outcome in outcomes or []:
        status = str(getattr(outcome, 'status', 'pending') or 'pending').lower()
        candidate_raw = getattr(outcome, 'candidate', original)
        candidate = (
            dict(candidate_raw)
            if isinstance(candidate_raw, dict)
            else dict(original)
        ...
        reason = str(getattr(outcome, 'reason', 'unknown') or 'unknown')
        if status in {'accepted', 'manual_review'}:
            # The queue write is intentionally before removing the source slot.
            # A later TimeoutError/refine_abort can therefore only leave a
            # duplicate pending item, never lose an accepted result.
            if _enqueue_refine_result(
                task_state,
                candidate,
                outcome=status,
            ):
        ...
        if status == 'rejected':
            ...
            _record_terminal_candidate_state(task_state, candidate, status)
            # 拒绝终态必须同步从权威切片快照移除并广播（与单 outcome 路径一致）
            removed_listed = _prune_rejected_listed_clips(task_state, candidate)
            for listed_key in removed_listed:
                listed_room = str(listed_key).split(':', 1)[0]
                if broadcast is not None:
            ...
            continue
        # 非终态（pending_lookahead）：保留在原槽或重新入队，等待续扫
        terminal_all = False
        candidate['_last_audit_dur'] = float(current_duration)
        if slot_open:
            pending[candidate_index] = candidate
            slot_open = False
        else:
            pending.append(candidate)
    if slot_open:
        # 全部 outcome 均为终态：弹出原槽位
        pending.pop(candidate_index)
    runtime_state = task_state.get("ocr_runtime_state")
    if isinstance(runtime_state, dict):
        runtime_state["broadcast_pending_rounds"] = list(pending)
...
def _deliver_audit_outcomes_on_cancel(
    sink: list[Any],
    pending: list[dict[str, Any]],
    candidate_index: int,
    produced_audited: list[dict[str, Any]],
    ...
    *,
    ...
    """审计被中止（超预算/抢占/停止）时交付已定稿结论。

    只有 ``rejected`` 是必须立刻生效的终态——它要把已经入列的脏切片剪掉，否则
    用户列表会长期停留"未审计粗边界"（纯回放片段、跨回合超长片段）。
    ``accepted``/``manual_review`` 在预算耗尽时不下发：它们必须伴随入点密扫
    边界，交由下一轮由审计缓存复现后走完整路径（保持"accepted 必带密扫"的
    既有不变量）。被降级项以 pending 回写，保证消费端不会把残缺批次当成整批
    终态而弹掉槽位（未审计的子候选不得静默丢弃）。
    """
    outcomes = [item for item in (sink or []) if item is not None]
    if not outcomes:
        return 0
    ...
    for outcome in outcomes:
        ...
        if status == 'rejected':
            deliverable.append(outcome)
            rejected += 1
        elif isinstance(candidate, dict):
            deliverable.append(BroadcastAuditOutcome(
                status='pending',
                candidate=dict(candidate),
                reason='cancelled_retry',
                retry_after_duration=float(current_duration),
            ))
    if not deliverable:
        return 0
    _consume_broadcast_audit_outcome_batch(
        pending,
        candidate_index,
        deliverable,
        produced_audited,
    ...
    if rejected:
        _log.warning(
            "边界审计被中止，已定稿拒绝结论照常交付: room_id=%s, rejected=%d, 其余留队续扫",
            task_state.get('room_id', ''),
            rejected,
        )
...
def _continuous_scan_metrics(
    *,
    scan_in: float,
    scan_out: float,
    analyzed_before: float,
    analyzed_after: float,
    recorded_before: float,
    recorded_after: float,
    scan_wall_sec: float,
) -> dict[str, float]:
    """Calculate gross scan cost separately from net coverage progress."""
    wall = max(0.0, float(scan_wall_sec))
    gross = max(0.0, float(scan_out) - float(scan_in))
    newly_covered = max(0.0, float(analyzed_after) - float(analyzed_before))
    backlog_before = max(0.0, float(recorded_before) - float(analyzed_before))
    backlog_after = max(0.0, float(recorded_after) - float(analyzed_after))
    lag_delta = backlog_after - backlog_before
    return {
        'gross_scanned_sec': round(gross, 3),
        'newly_covered_sec': round(newly_covered, 3),
        'gross_scan_throughput': round(gross / wall, 3) if wall > 0.0 else 0.0,
        'net_coverage_throughput': (
            round(newly_covered / wall, 3) if wall > 0.0 else 0.0
        ),
        'backlog_before_sec': round(backlog_before, 3),
        'backlog_after_sec': round(backlog_after, 3),
        'lag_delta_sec': round(lag_delta, 3),
        'lag_slope': round(lag_delta / wall, 3) if wall > 0.0 else 0.0,
    }


def _continuous_throughput_history(task_state: dict[str, Any]) -> list[float]:
    """Prefer net coverage history; fall back to legacy gross history."""
    net = [
        float(value)
        for value in (task_state.get('net_coverage_throughput_history') or [])
        if float(value) > 0.0
    ]
    if net:
        return net[-5:]
    return [
        float(value)
        for value in (task_state.get('scan_throughput_history') or [])
        if float(value) > 0.0
    ][-5:]


def _count_broadcast_ready_candidates(
    pending: list[dict[str, Any]],
    current_dur: float,
) -> int:
    """Count candidates whose lookahead rear-window is fully recorded.

    Mirrors the ready/probe classification used by the refine worker: a ready
    candidate can be finalized with the evidence already on disk, so it should
    not be starved merely because coarse coverage is still catching up.
    """
    count = 0
    try:
        duration = float(current_dur or 0.0)
    except (TypeError, ValueError):
        return 0
    for candidate in pending:
        ...
        try:
            ...
            continue
        if candidate.get("_audit_continue_ready"):
            count += 1
            continue
        has_strong = candidate.get("result_ts") is not None or str(
            candidate.get("end_by", "") or ""
        ).lower() in ("buy_phase", "next_prep")
        needed = 25.0 if has_strong else 40.0
        if duration >= end + needed:
            count += 1
    return count


def _should_run_broadcast_refine(
    backlog_sec: float,
    *,
    finalizing: bool = False,
    pending_count: int = 0,
    refine_elapsed_sec: float = 0.0,
    ready_count: int = 0,
) -> bool:
    """Allow bounded broadcast refinement without blocking coarse coverage.

    Coarse OCR is the only stage that advances the continuous-analysis
    coverage cursor. Refinement has an independent async slot and uses the
    shared ONNX resource only for one bounded candidate step, so pending
    candidates must not keep refinement enabled while the recorder is already
    behind. However, a candidate whose full rear-window is already recorded
    must be allowed to finalize even during catch-up; otherwise audit can be
    starved forever behind an ever-growing backlog.
    """
    if finalizing:
        return True
    if float(refine_elapsed_sec) >= _BCAST_REFINE_KEEP_MAX_SEC:
        return False
    if int(ready_count or 0) > 0 and int(pending_count or 0) > 0:
        return True
...
def _select_scan_window(
    ...
    retry_range: tuple[float, float] | list[float] | None = None,
    *,
    finalize: bool = False,
) -> tuple[tuple[float, float], bool, int, bool, str]:
    """计算持续分析的扫描窗口（单点预算入口，消除二次覆盖）。
    正常路径仅调用一次插件的 plan_scan_window()，使吞吐历史和自适应
    追赶预算生效，禁止再次被兼容预算覆盖。
    返回: (scan_range, use_ocr_this_tick, scan_timeout, full_rescan, scan_reason)
    """
    last_analyzed = float(plan_state.get('last_analyzed', 0.0) or 0.0)
    # 1. 超时重试窗口：优先级最高（finalize 阶段由外部收尾逻辑覆盖）
    if (
        isinstance(retry_range, (list, tuple))
        and len(retry_range) >= 2
        and float(retry_range[1]) > float(retry_range[0])
        and not finalize
    ):
        scan_range = (
            round(max(0.0, float(retry_range[0])), 3),
            round(float(retry_range[1]), 3),
        )
        use_ocr = True
        scan_timeout = _window_scan_timeout(
            scan_range[1] - scan_range[0],
            use_ocr=True,
        )
        full_rescan = False
        scan_reason = 'timeout_retry'
        return scan_range, use_ocr, scan_timeout, full_rescan, scan_reason
    # 2. 正常路径：自适应插件规划
    if hasattr(analyzer, 'plan_scan_window') and getattr(analyzer.capabilities(), 'realtime_continuous', False):
        window = analyzer.plan_scan_window(plan_state, current_dur, pressure or {})
        scan_range = (float(window.start_sec), float(window.end_sec))
        use_ocr = bool(window.use_ocr)
        scan_timeout = int(window.timeout_sec)
        full_rescan = bool(plan_state.get('full_rescan', last_analyzed <= 0.0))
        new_media = max(0.0, scan_range[1] - last_analyzed)
        backlog = max(0.0, current_dur - last_analyzed)
        if backlog > 45.0 or new_media > 35.0:
            scan_reason = 'catchup'
        else:
            scan_reason = 'realtime_incremental'
        return scan_range, use_ocr, scan_timeout, full_rescan, scan_reason
    # 3. 兜底路径（通用/非实时连续分析器）
    mode = str(plan_state.get('mode') or 'generic')
    scan_range, use_ocr, scan_timeout, full_rescan = _continuous_valorant_scan_budget(
...
def _analyze_scene_or_rounds(
    ...
    对一次性分析（录制已结束的完整文件）：
    - game="valorant" 时经 AnalyzerRegistry → OCR 粗定位；broadcast profile
      追加赛事阶段审计；
    - POV 无结果时仍回退到 generic scene，broadcast 无安全候选时禁止回退。
    Returns:
    ...
    if game == "valorant":
        try:
            ...
            if highlights:
                ...
                return highlights
            if valorant_profile == "broadcast":
                _log.warning(
                    "Valorant broadcast 审计无安全候选，禁止回退通用场景分析: %s",
                    os.path.basename(video_path),
                )
                return []
            _log.info("Valorant OCR 检测无结果，回退到场景检测")
        except Exception as exc:
            _log.warning("Valorant OCR 检测失败: %s", exc)
            if valorant_profile == "broadcast":
                return []
...
def _epoch_ms_to_mono(epoch_ms: float) -> float:
    """将前端 Date.now() 毫秒时间转换为后端 time.monotonic()。"""
    return time.monotonic() - (time.time() - float(epoch_ms) / 1000.0)
def _compute_recording_to_preview_delta(
    preview_time: float,
    sample_mono: float,
    recording_start_mono: float,
) -> float:
    """计算 preview_local = recording_local + delta 中的 delta。"""
    recording_elapsed = float(sample_mono) - float(recording_start_mono)
    return float(preview_time) - recording_elapsed
...
# 时间线直播 DVR 选项：0 表示关闭历史回放，仅保留播放器所需的短安全缓存。
_TIMELINE_REPLAY_OPTIONS_SEC = (0, 120, 300, 600)
_DEFAULT_TIMELINE_REPLAY_SECONDS = 300
def _normalize_timeline_replay_seconds(value: object) -> int:
    if value is None or isinstance(value, bool) or value == '':
        return _DEFAULT_TIMELINE_REPLAY_SECONDS
    try:
    ...
    return normalized if normalized in _TIMELINE_REPLAY_OPTIONS_SEC else _DEFAULT_TIMELINE_REPLAY_SECONDS
def load_settings():
    ...
    if os.path.exists(SETTINGS_FILE):
        try:
            ...
            # 缓存被显式置 None（如 handle_save_settings 的失效路径）时不得走
            # mtime 相等捷径返回 None——否则 load_settings 从此永久返回 None。
            if mtime == _settings_cache_mtime and _settings_cache is not None:
    ...
    _settings_cache = {
        ...
        'audio_bitrate': '128k',
        'preview_quality': '高清',
        'timeline_replay_seconds': _DEFAULT_TIMELINE_REPLAY_SECONDS,
        'dvr_mirror_enabled': True,
...
def _apply_dvr_mirror_from_settings(settings: dict) -> None:
    """将 settings.json 的录制镜像开关同步到运行时 LscConfig 单例。

    StreamCapture 追加第二个输出前读的是 load_config().dvr_mirror_enabled，
    而设置页写的是 settings.json；二者不同步会让开关对运行时无效
    （与 shared_ingest_enabled 同一模式）。
    """
    if 'dvr_mirror_enabled' not in settings:
        return
    try:
        enabled = bool(settings.get('dvr_mirror_enabled'))
        cfg = load_config()
        if bool(getattr(cfg, 'dvr_mirror_enabled', True)) != enabled:
            cfg.dvr_mirror_enabled = enabled
            _log.info("运行时 dvr_mirror_enabled 已同步为 %s", enabled)
    except Exception as exc:
        _log.warning("同步 dvr_mirror_enabled 到 LscConfig 失败: %s", exc)
...
def save_settings(settings: dict):
    ...
    try:
        ...
        settings['timeline_replay_seconds'] = _normalize_timeline_replay_seconds(
            settings.get('timeline_replay_seconds')
        )
        ...
        _apply_dvr_mirror_from_settings(settings)
...
def _push_mse_segment(
    ...
    方案 A（2026-09-11）：回看不经后端流通道，MSE 只剩直播单通道，
    故不再有通道路由参数与文件流背压豁免。
    """
    normalized_kind = {
        "mse_init": "init",
    ...
    if normalized_kind == "init" and room_id not in _mse_live_phase:
        # phase 'streaming' 延迟到首个 init 段产出。
...
def _room_to_dict(room: Any, *, redact_sensitive: bool = True) -> dict[str, Any]:
    ...
    return {
        ...
        # 录制镜像（录制中本地回看的数据源）；无镜像时为空串，前端回退记录文件本身。
        'dvr_output_path': getattr(room, 'dvr_output_path', '') or '',
        ...
        'recording_to_preview_delta': getattr(room, 'recording_to_preview_delta', None),
        'preview_clock_epoch_id': getattr(room, 'preview_clock_epoch_id', '') or '',
...
def restore_persisted_rooms(manager: RoomOrchestrator) -> int:
    ...
    def _restore_on_orchestrator_thread() -> int:
        ...
        for item in saved_rooms:
            ...
            # 恢复平台、主播名与直播标题记忆
            for field in ("streamer_name", "platform", "platform_name", "stream_title"):
                value = item.get(field)
...
def register_room_handlers(server, bridge):
    ...
    try:
        _apply_dvr_mirror_from_settings(load_settings())
    except Exception as exc:
        _log.debug("启动同步 dvr_mirror 失败: %s", exc)
    # rooms_updated 广播节流：首次立即发送，300ms 内合并后续更新
    ...
    async def _shared_mse_on_error(room_id: str, err: str, loop) -> None:
        ...
        # 旧播放器可能在用户暂停时留下 push pause；重连必须从新流开始推送。
        _clear_mse_push_paused(room_id)
            ...
            if _shared_preview_reconnect_ready(result, ingest):
                ...
                def _rotate_shared_epoch_on_reconnect():
                    ...
                    if room is None:
                        return False
                    new_epoch = uuid4().hex
                    _set_preview_epoch(room, new_epoch) # preview_epoch_id = new_epoch
    ...
    def _queue_recording_size_patches(*_args, **_kwargs):
        ...
        for room in manager.list_rooms():
            ...
            patches.append({
                ...
                'is_recording_starting': room_id in _recording_starting,
                # last_error 可能携带 CDN 签名 URL，与全量快照（:3011）同样过 redact
                'last_error': redact_text(getattr(room, 'last_error', '') or ''),
    ...
    bus = manager.bus
    bus.subscribe("room_connect_finished", lambda *_a: (_queue_rooms_update(), _persist_current_rooms(manager)))
    ...
    def _on_manager_recording_stopped_offline(room_id: str, reason: str, message: str) -> None:
        """录制侧重连判定下播时，把预览降级为「本地文件回看」（不再启动文件流）。"""
        if reason != 'offline':
            return
        loop = _capture_ws_loop()
        if loop is None or not loop.is_running():
            _log.debug(
                "skip offline degraded preview: no running WS loop (room_id=%s)",
                room_id,
            )
            return
        asyncio.run_coroutine_threadsafe(
            _degrade_preview_offline(
                server,
                manager,
                bridge,
                room_id,
    ...
    async def handle_connect(websocket):
        """新客户端连接时推送当前房间列表与设置。
        持久化房间已在 ``main.py`` 启动 WebSocket 前恢复到 manager；这里不再重复
        从磁盘加载，避免前端重连时重复创建房间。若本进程内已有房间（例如前端
        热重连），则推送内存中的当前列表。
        """
        _capture_ws_loop()
        # 推送启动阶段已经恢复好的 manager 房间
        ...
        try:
            ...
            saved['timeline_replay_seconds'] = _normalize_timeline_replay_seconds(
                saved.get('timeline_replay_seconds')
            )
            await websocket.send(json.dumps({
    ...
    async def handle_add_room(data):
        ...
        if room is None:
            ...
            existing_urls = [
                getattr(r, "room_url", "").strip().rstrip("/").lower()
                for r in manager.list_rooms()
            ]
            if url_norm in existing_urls:
                _log.warning("添加房间失败（房间已存在）: url=%s", redact_url(url))
                return {'success': False, 'error': '该直播间已在列表中，无需重复添加'}
            _log.warning("添加房间失败（达上限）: url=%s", redact_url(url))
            return {'success': False, 'error': '房间数已达上限（最多支持 12 间）'}
        _broadcast_rooms()
    ...
    async def handle_remove_room(data):
        ...
        # 房间被移除：终态权威快照一并清掉，避免房间重新加入后旧快照被当成权威集合。
        _clear_authority_snapshot(room_id)
        stale_streamer = _preview_stream_registry().pop(room_id)
        if stale_streamer is not None:
    ...
    @server.on('set_preview_clock')
    async def handle_set_preview_clock(data):
        """记录直播 MSE 当前 epoch 的录制轴→预览轴映射。
        前端同时发送 video.currentTime 与 Date.now()；后端用同一台机器的
        monotonic 时钟换算录制文件已运行时长，从而得到不依赖固定延迟的映射。
        该映射只在当前 recording_id / preview_epoch_id 内有效。
        """
        room_id = str(data.get('room_id') or '').strip()
        preview_epoch_id = str(data.get('preview_epoch_id') or '').strip()
        preview_time = _safe_float(data.get('preview_current_time'), -1.0)
        sample_epoch_ms = _safe_float(data.get('sample_epoch_ms'), 0.0)
        if not room_id:
            return {'success': False, 'error': 'room_id is required'}
        if preview_time < 0.0 or sample_epoch_ms <= 0.0:
            return {'success': False, 'room_id': room_id, 'accepted': False, 'reason': 'invalid_sample'}
        now_epoch = time.time()
        sample_age = now_epoch - sample_epoch_ms / 1000.0
        if abs(sample_age) > _PREVIEW_CLOCK_MAX_SAMPLE_AGE_SEC:
            return {'success': False, 'room_id': room_id, 'accepted': False, 'reason': 'stale_sample'}
        sample_mono = _epoch_ms_to_mono(sample_epoch_ms)
        sample_updated_mono = time.monotonic()
        def _set_clock():
            room = manager.get_room(room_id)
            if room is None:
                return {'accepted': False, 'reason': 'room_not_found'}
            if not room.preview_enabled or getattr(room, 'preview_mode', 'live_mse') != 'live_mse':
                return {'accepted': False, 'reason': 'preview_not_live'}
            current_epoch = str(getattr(room, 'preview_epoch_id', '') or '')
            if current_epoch and preview_epoch_id and current_epoch != preview_epoch_id:
                return {
                    'accepted': False,
                    'reason': 'preview_epoch_mismatch',
                    'preview_epoch_id': current_epoch,
                }
            if not room.is_recording and not getattr(room, 'is_reconnecting', False):
                return {'accepted': False, 'reason': 'recording_not_active'}
            recording_start = _recording_media_start(room)
            if recording_start <= 0.0:
                return {'accepted': False, 'reason': 'recording_clock_unavailable'}
            recording_elapsed = sample_mono - recording_start
            if recording_elapsed < -5.0:
                return {'accepted': False, 'reason': 'sample_before_recording'}
            delta = _compute_recording_to_preview_delta(
                preview_time,
                sample_mono,
                recording_start,
            )
            if abs(delta) > _PREVIEW_CLOCK_MAX_ABS_DELTA_SEC:
                return {'accepted': False, 'reason': 'delta_out_of_range'}
            actual_epoch = current_epoch or preview_epoch_id
            room.recording_to_preview_delta = float(delta)
            room.preview_clock_epoch_id = actual_epoch
            room.preview_clock_anchor_mono = sample_mono - preview_time
            room.preview_clock_anchor_time = preview_time
            room.preview_clock_updated_mono = sample_updated_mono
            _log.info(
                "预览时钟校准: room=%s, epoch=%s, recording_elapsed=%.3f, preview=%.3f, delta=%.3f",
                room_id,
                actual_epoch,
                recording_elapsed,
                preview_time,
            ...
            return {
                'accepted': True,
                'recording_to_preview_delta': float(delta),
                'preview_clock_epoch_id': actual_epoch,
                'recording_id': str(getattr(room, 'recording_id', '') or ''),
                'sample_epoch_ms': sample_epoch_ms,
            }
        result = await asyncio.get_running_loop().run_in_executor(
            _bridge_executor,
            lambda: bridge.manager.call(_set_clock),
        )
    ...
    async def handle_start_recording_review(data):
        """已废弃（方案 A，2026-09-11）：回看不再经后端流通道。

        保留声明式桩，避免旧前端版本误用后**静默失败**：回看 = 前端直接播本地录制文件
        （录制中用 record_output_path 同目录的 .dvr.mp4 镜像，否则用 record_output_path）。
        """
        _log.info(
            "start_recording_review 已废弃（回看改前端本地文件播放）: room_id=%s",
            (data or {}).get('room_id'),
        )
        return {
            'success': False,
            'error': 'deprecated: use local file playback',
            'room_id': (data or {}).get('room_id'),
        }
    @server.on('close_recording_review')
    async def handle_close_recording_review(data):
        """已废弃（方案 A，2026-09-11）：退出回看改为前端本地通道切换，后端无状态可清。"""
        _log.info(
            "close_recording_review 已废弃（回看改前端本地文件播放）: room_id=%s",
            (data or {}).get('room_id'),
        )
        return {
            'success': False,
    ...
    async def handle_set_mark_in(data):
        ...
        # 支持显式传入精确计算的 wallclock（如回看或拖动微调）；未传入时 live=True 取当前 monotonic
        wallclock_val = data.get('wallclock')
        if wallclock_val is not None:
            captured_wallclock = float(wallclock_val)
        else:
    ...
    async def handle_save_settings(data):
        ...
        # 只清缓存不清 mtime 会让下一次 load_settings 命中「mtime 相等」捷径
        # 直接返回 None 并永久卡死；mtime 一并失效才会真正重读磁盘。
        _settings_cache_mtime = 0.0
        _log.info(
            "设置已保存: output_dir=%s, shared_ingest_enabled=%s, timeline_replay_seconds=%s",
            data.get('output_dir', ''),
            data.get('shared_ingest_enabled'),
            data.get('timeline_replay_seconds'),
        )
        return {'success': True}
    ...
    async def handle_check_dependencies(data):
        ...
        def _probe_dependencies() -> dict:
            # 子进程探测（2×5s 超时 + NVENC 探针）整体放 executor：
            # 缓存过期后的冷调用在事件循环内联执行会冻结所有房间消息 10s+
            from lsc.core.services.mse_streamer import _check_nvenc
            ...
            _log.info("检测依赖: ffmpeg=%s, ffprobe=%s, nvenc=%s", ffmpeg_path, ffprobe_path, nvenc_ok)
            return {'success': True, 'dependencies': results}
        result = await asyncio.get_running_loop().run_in_executor(_bridge_executor, _probe_dependencies)
    ...
    async def _handle_mse_preview(srv, mgr, room_id: str, enabled: bool, data: dict | None = None, *, force_restart: bool = False) -> dict[str, Any]:
        ...
        """
        if enabled:
            # 新建/恢复预览前清理旧播放器遗留的用户暂停或背压状态。
            _clear_mse_push_paused(room_id)
            # Check if already streaming / starting
            existing = _preview_stream_registry().get(room_id)
            if existing is not None and force_restart:
                _log.info("强制重启预览流以切回直播: room_id=%s", room_id)
                _preview_stream_registry().pop(room_id)
                try:
                    await asyncio.get_running_loop().run_in_executor(_bridge_executor, existing.stop)
                except Exception as exc:
                    _log.debug("existing preview stop failed: %s", exc)
                def _reset_room_preview_live():
                    r = mgr.get_room(room_id)
                    if r is not None:
                        r.preview_mode = 'live_mse'
                    return True
                await asyncio.get_running_loop().run_in_executor(_bridge_executor, lambda: bridge.manager.call(_reset_room_preview_live))
                existing = None
            ...
            try:
                ...
                async def _on_mse_error(room_id: str, err: str, loop):
                    ...
                    async def _finalize_mse_error(error_text: str, reason: str, timeline_reason: str) -> None:
                        ...
                        if reason == 'offline':
                            await _degrade_preview_offline(
                                srv,
                                mgr,
                                bridge,
                                room_id,
                        ...
                        if success:
                            ...
                            def _rotate_epoch_on_reconnect():
                                ...
                                if room is None:
                                    return False
                                new_epoch = uuid4().hex
                                _set_preview_epoch(room, new_epoch)
                                get_timeline_service().on_preview_epoch_change(room_id, new_epoch)
                                return True
                            try:
                                await loop.run_in_executor(
    ...
    async def handle_request_mse_init(data):
        ...
        """
        room_id = (data or {}).get('room_id')
        channel = (data or {}).get('channel') or 'live'
        if not room_id:
            return {'success': False, 'error': 'room_id is required'}
        _log.debug("请求 MSE init 重发: room_id=%s, channel=%s", room_id, channel)
        # 方案 A：回看不再走后端流通道，MSE 只剩直播单通道（channel 仅回显给旧前端）。
        ...
        _log.debug("request_mse_init: room_id=%s, ok=%s, channel=%s", room_id, ok, channel)
        return {
            'success': ok,
            'room_id': room_id,
            'channel': channel,
            'note': 'init replayed' if ok else 'init not ready yet',
        }
    ...
    async def handle_mse_backpressure(data):
        ...
        _log.debug(
            ...
            room_id, state, (data or {}).get('pending'), paused,
        )
    ...
    def _reset_epoch_scoped_clip_state(room_id: str, prev_state: dict[str, Any] | None = None) -> int:
        """新录制 epoch：清空上一会话按房间登记的切片键位与精修冻结。

        listed_clips / _listed_clip_ids / _listed_clip_bounds / _exported_clip_ids /
        _deleted_clip_keys 均以 ``room:round_key`` 为键；新录制 epoch 的同号
        round_key 是完全不同的回合，旧登记必须清理，否则旧切片会混入权威快照
        （前端/剪映草稿）或压制新切片的入列广播。
        """
        with _analysis_jobs_lock:
            removed = _strip_room_scoped_keys(
                room_id,
                _listed_clip_ids,
                _listed_clip_bounds,
                _exported_clip_ids,
                _deleted_clip_keys,
                # 标签记忆同样按 room:round_key 作用域：新 epoch 的同号 round_key
                # 必须重新分配标签，否则会复用上一场录像的标签
                _listed_clip_labels,
            )
            if isinstance(prev_state, dict):
                prev_state['listed_clips'] = {}
        # 精修冻结键已改为 ``room:round_key`` 作用域，仅清理本房间的旧 epoch 冻结，
        # 不影响其他并发分析任务的冻结边界
        with _refined_round_keys_lock:
            removed += _strip_room_scoped_keys(room_id, _refined_round_keys)
        return removed
    ...
    async def _auto_export_highlights(main_room, target_rooms, highlights, job_prefix, preset_id='',
        ...
        if not highlights or not target_rooms:
            return []
        # C2：扫描通路的审计终态必须同时进 durable 账本（幂等，按 round_key）。
        # 现场：20:43:26 round-000105 定稿（audit=passed）只进了 listed_clips，
        # accepted/rejected + audit_terminal_total 停在 4，收尾 sidecar 因此缺 105，
        # 导出时权威回落到 20:37 的旧分析 sidecar，被改回 pending_lookahead 后按"未确认"跳过。
        _project_scan_audit_terminals_for_room(
            str(getattr(main_room, 'room_id', '') or ''), highlights,
        )
        # L2：定稿后组内择一（同一真实回合被多条候选认领时只留一条；原因码可辨）
        _kept_highlights, _merged_highlights = _dedupe_overlapping_rounds(highlights)
        if _merged_highlights:
            _mark_merged_rounds(
                str(getattr(main_room, 'room_id', '') or ''),
                _merged_highlights,
                broadcast=bridge.queue_broadcast,
            )
            highlights = _kept_highlights
        main_rec = _recording_media_start(main_room)
        ...
        for idx, hl in enumerate(highlights):
            ...
            for target_room in target_rooms:
                ...
                if list_only:
                    ...
                    if action == "skip":
                        continue
                    label = _resolve_ai_clip_label(
                        listed_key, rid, room_name, round_idx,
                        is_first=(action == "first"),
                    )
                    ...
                    clip_payload = {
                            ...
                            'round_key': round_key,
                            'recording_id': str(getattr(target_room, 'recording_id', '') or ''),
                ...
                if listed_key in _exported_clip_ids:
                    continue
                label = _resolve_ai_clip_label(listed_key, rid, room_name, round_idx, is_first=True)
                job_id = f"auto-{job_prefix}-{round_key}-{rid}"
                clip_id = _clip_id(rid, export_start, export_end)
                if defer_export:
    ...
    async def _flush_deferred_exports(force: bool = False) -> int:
        ...
        for job in jobs:
            try:
                ...
                # 单项异常不得吞掉剩余任务：queue_export 抛异常时本项上报失败
                # 并解除已导出去重标记（否则切片永久显示「已入列」且无法重试），
                # 其余任务继续处理。
                _log.error("延后导出抛异常: room=%s, job=%s, error=%s",
                           job['room_id'], job['job_id'], exc)
                _exported_clip_ids.pop(job.get('listed_key', ''), None)
                bridge.queue_broadcast({
                    'type': 'clip_failed',
                    'data': {
                        'room_id': job['room_id'],
                        'job_id': job['job_id'],
                        'error': f"延后导出任务异常: {exc}",
                    },
                })
                continue
            if result.get('success'):
    ...
    async def _continuous_valorant_worker(
        ...
        _continuous_tasks, _analysis_semaphore, _analysis_thread_semaphore,
        _bridge_executor,
        ...
        try:
            while True:
                ...
                # recording_id 比路径更可靠：收尾改名可以继续消费，重连
                # 复用同一路径则必须中止旧 epoch，避免结果写回新文件。
                scan_recording_id = str(task_state.get('recording_id') or '')
                ...
                _finalize_boundary_pass = bool(
                    task_state.get('finalize_boundary_pass', _finalizing)
                )
                ...
                rotation_cancel_logged = False
                def _scan_cancel_check():
                    nonlocal rotation_cancel_logged
                    with _analysis_jobs_lock:
                        st = _continuous_tasks.get(room_id, {})
                        cancelled = bool(st.get('cancelled') or st.get('scan_abort'))
                        current_recording_id = str(st.get('recording_id') or '')
                    if (
                        not cancelled
                        and scan_recording_id
                        and current_recording_id
                        and current_recording_id != scan_recording_id
                    ):
                        if not rotation_cancel_logged:
                            rotation_cancel_logged = True
                            _log.info(
                                "持续分析检测到录制 epoch 切换，提前中止旧文件扫描: "
                                "room_id=%s, old_epoch=%s, new_epoch=%s",
                                room_id,
                                scan_recording_id,
                ...
                def _do_scan(
                    ...
                    _vp = resolve_real_video_path(_vp)
                    ...
                    if game == 'valorant' and _mode == 'valorant_round':
                        ...
                        _scan_state = {
                            ...
                            'current_dur': _dur,
                            'finalize': _finalize_boundary_pass,
                            'valorant_profile': str(task_state.get('valorant_profile') or 'pov'),
                            'ocr_sample_interval': float(task_state.get('ocr_sample_interval', 1.0)),
                            'realtime_fast_mode': bool(task_state.get('realtime_fast_mode', False)),
                            'deferred_audit': not _finalizing,
                        }
                        _rounds = plugin.scan_window(
                            _vp, _window, _scan_state, cancel_check=_cancel,
                        )
                        return {
                            'rounds': _rounds or [],
                            'scan_succeeded': bool(_scan_state.get('scan_succeeded', True)),
                        }
                ...
                # 粗扫与审计解耦（A-05）：审计在独立 _refine_semaphore 上运行，
                # 不再占用粗扫的 asyncio semaphore。粗扫 kick 时根据实时 backlog
                # 决定是否抢占审计：
                #   - backlog > _REFINE_PREEMPT_BACKLOG_SEC 或审计超预算：置
                #     refine_abort，审计经 cancel_check 数秒内退出并释放线程
                #     semaphore，粗扫立即前进（旧实现会让一次审计批阻塞覆盖
                #     60–90s，滞后永远追不平）；
                #   - 低滞后窗口：等待审计跑完一个批次（受 _BCAST_REFINE_KEEP_MAX_SEC
                #     硬上限约束），保证审计能完整定稿而不是被无限饿死。
                _refine_started_at = float(
                    task_state.get('refine_started_at')
                    or task_state.get('refine_requested_at')
                    or 0.0
                )
                _refine_elapsed = (
                    max(0.0, time.time() - _refine_started_at)
                    if _refine_started_at > 0.0
                    else float('inf')
                )
                _refine_active_now = bool(
                    task_state.get('refine_running') or task_state.get('refine_pending')
                )
                _backlog_now = max(
                    0.0,
                    float(task_state.get('recorded_duration') or 0.0)
                    - float(scan_range[1] or 0.0),
                )
                task_state['refine_abort'] = bool(
                    _refine_active_now
                    and (
                        _backlog_now > _REFINE_PREEMPT_BACKLOG_SEC
                        or _refine_elapsed >= _BCAST_REFINE_KEEP_MAX_SEC
                    )
                    and not task_state.get('refine_fairness_active')
                )
                ...
                try:
                    ...
                    async with _analysis_semaphore:
                        ...
                        fut = loop.run_in_executor(_ai_executor, _run_scan_guarded)
                        try:
                            scan_payload = await asyncio.wait_for(
                                asyncio.shield(fut),
                                timeout=scan_timeout,
                            )
                            if isinstance(scan_payload, dict) and 'rounds' in scan_payload:
                                result = scan_payload.get('rounds') or []
                                scan_succeeded = bool(scan_payload.get('scan_succeeded'))
                            else:
                                result = scan_payload or []
                                scan_succeeded = True
                            if not scan_succeeded:
                                # 取消路径（停止/录制 epoch 切换）会令 OCR 检测器置
                                # scan_succeeded=False；停止时这是正常退出，不能当成
                                # “读取失败”触发无意义的窗口重试日志/状态。
                                with _analysis_jobs_lock:
                                    _cancelling_now = bool(
                                        task_state.get('cancelled')
                                        or task_state.get('scan_abort')
                                    )
                                if _cancelling_now:
                                    _log.info(
                                        "持续分析扫描被取消，窗口不再重试: room_id=%s",
                                        room_id,
                                    )
                                ...
                                # 完整 traceback（run6/run7 里都能看到），把日志噪音当成异常。
                                # 这里单独分类：不抛异常，记一条简短 warning 后继续下一轮。
                        ...
                        scan_result_container['scan_succeeded'] = True
                        scan_result_container['video_path'] = video_path
                        scan_result_container['recording_id'] = scan_recording_id
                        scan_result_container['current_dur'] = completed_dur
                        scan_result_container['scan_range'] = (
                        ...
                        # 密扫与 broadcast 审计改后台（A-03）：粗结果先入列并推进游标，
                        # 随后在后台执行物理入点密扫与视觉审计，避免阻塞粗扫主循环。
                        _is_bcast = str(task_state.get('valorant_profile') or 'pov') == 'broadcast'
                        _rs_state = task_state.setdefault('ocr_runtime_state', {})
                        _bcast_pending_cnt = len(_rs_state.get('broadcast_pending_rounds') or [])
                        task_state['audit_queue_depth'] = _bcast_pending_cnt
                        _need_refine = [
                            r for r in (result or [])
                            if isinstance(r, dict) and r.get('boundary_refined') is False
                        ] or (_is_bcast and _bcast_pending_cnt > 0)
                        # 已明显滞后时跳过密扫（优先追赶），优先追赶粗扫（密扫单回合可达 30–55s）。
                        # 扫描窗口可能耗时数十秒，`current_dur` 是 kick 时的快照，
                        # 不能用它判断当前是否已进入 priority/degraded catchup。
                        # 优先读取主循环近期写入的 current/recorded duration，
                        # 防止追赶期间仍启动后台密扫占用分析槽。
                        _live_current_dur = max(
                            float(current_dur or 0.0),
                            float(task_state.get('current_dur') or 0.0),
                            float(task_state.get('recorded_duration') or 0.0),
                        )
                        _lag_now = max(0.0, _live_current_dur - float(completed_dur))
                        _bcast_ready_cnt = _count_broadcast_ready_candidates(
                            _rs_state.get('broadcast_pending_rounds') or [],
                            _live_current_dur,
                        )
                        _allow_background_refine = _should_run_broadcast_refine(
                            _lag_now,
                            finalizing=_finalizing,
                            pending_count=_bcast_pending_cnt,
                            ready_count=_bcast_ready_cnt,
                        )
                        _coarse_since_refine = int(
                            task_state.get('coarse_windows_since_refine') or 0
                        )
                        _refine_fairness_due = bool(
                            _is_bcast
                            and _bcast_pending_cnt > 0
                            and (
                                _bcast_ready_cnt > 0
                                or _coarse_since_refine >= _REFINE_FAIRNESS_COARSE_WINDOWS
                            )
                        )
                        if _is_bcast and _bcast_pending_cnt > 0:
                            task_state['coarse_windows_since_refine'] = _coarse_since_refine + 1
                        _refine_task = task_state.get('refine_task')
                        _refine_active = bool(
                            _refine_task is not None and not _refine_task.done()
                        )
                        if (
                            _need_refine
                            and game == 'valorant'
                            and mode == 'valorant_round'
                            # 收尾期间默认不开新审计；但赛事分支若仍有待审计候选，
                            # 必须放行——否则候选永远等不到 finalize=True 的那次审计，
                            # 只能被 _FINALIZE_TAIL_STALL_MAX_ROUNDS 兜底强制定稿
                            # （实测 2026-09-11：收尾 10 分钟 0 帧推理，候选卡 pending，
                            # 收尾判定无限「继续补扫」）。_should_run_broadcast_refine
                            # 在 finalizing 时本就返回 True。
                            and (
                                not _finalizing
                                or (_is_bcast and _bcast_pending_cnt > 0)
                            )
                            ...
                            and not task_state.get('stop_tail_scan')
                            and (_allow_background_refine or _refine_fairness_due)
                            and not _refine_active
                        ):
                            _refine_vp = video_path
                            _refine_rounds = [dict(r) for r in (result or [])]
                            _refine_dur = completed_dur
                            _refine_profile = str(
                                task_state.get('valorant_profile') or 'pov'
                            )
                            async def _boundary_refine_bg(
                                _vp=_refine_vp,
                                _rounds=_refine_rounds,
                                _dur=_refine_dur,
                                _profile=_refine_profile,
                            ):
                                def _publish_refine_results(_rounds: list[dict[str, Any]]) -> None:
                                    # 精修结果已经在 _enqueue_refine_result 中可靠入队。
                                    # 这里只负责唤醒主循环；绝不能再写共享的单值
                                    # scan_result_container，否则会覆盖尚未消费的粗扫结果。
                                    _pub_ev = None
                                    with _analysis_jobs_lock:
                                        _pub_st = _continuous_tasks.get(room_id)
                                        if _pub_st is not None:
                                            _pub_ev = _pub_st.get('scan_done_event')
                                    if _pub_ev is not None:
                                        try:
                                            _pub_ev.set()
                                        except Exception:
                                            _log.debug("scan_done_event.set 失败", exc_info=True)
                                    _log.info(
                                        "持续分析边界精修结果入消费通知: room_id=%s, %d 回合",
                                        room_id,
                                        len(_rounds),
                                    )
                                ...
                                def _do_boundary_refine():
                                    ...
                                    def _run_with_onnx_resource(fn, *args, **kwargs):
                                        """Serialize one bounded refine step only."""
                                        with _ffmpeg_capture_semaphore:
                                            with _onnx_inference_semaphore:
                                                return fn(*args, **kwargs)

                                    # 收尾阶段判定：录制已停止 / 收尾进行中时文件不再增长，
                                    # 后视窗口结构性不足的候选必须走 finalize 审计（以文件尾
                                    # 为边界直接定稿），否则会永久滞留待审计队列使收尾卡死。
                                    _finalize_now = bool(
                                        task_state.get('finalizing')
                                        or task_state.get('finalize_pending')
                                        or task_state.get('stop_requested')
                                        or str(task_state.get('finalization_state') or '')
                                        == 'finalizing'
                                    )
                                    if _profile == 'broadcast':
                                        ...
                                        _frame_provider = _rs_state.get('broadcast_frame_provider')
                                        if not isinstance(_frame_provider, FrameProvider):
                                            _frame_provider = FrameProvider(max_frames=512)
                                            _rs_state['broadcast_frame_provider'] = _frame_provider
                                        _pending = list(_rs_state.get('broadcast_pending_rounds') or [])
                                        if not _pending:
                                            return []
                                        ...
                                        while max_audit_quota > 0 and _pending:
                                            ...
                                            for idx, c in enumerate(_pending):
                                                c_end = float(c.get('end', 0.0))
                                                has_strong = c.get('result_ts') is not None or str(c.get('end_by', '')).lower() in ('buy_phase', 'next_prep')
                                                needed = 25.0 if has_strong else 40.0
                                                last_dur = float(c.get('_last_audit_dur') or 0.0)
                                                if c.get('_audit_continue_ready'):
                                                    ready_indices.append(idx)
                                                elif _dur >= c_end + needed:
                                                    ready_indices.append(idx)
                                                elif _finalize_now and _dur <= c_end:
                                                    # 收尾且候选终点已到/超出文件末尾：后视窗口
                                                    # 结构性不足。必须强制尝试一次终态审计
                                                    # （finalize=True），否则该候选的
                                                    # `_last_audit_dur` 被钉到文件时长后
                                                    # ready/probe 判定永不再满足，永久滞留。
                                                    ready_indices.append(idx)
                                                elif last_dur <= 0.0 and _dur >= c_end + 10.0:
                                                    probe_indices.append(idx)
                                                elif last_dur > 0.0 and _dur >= last_dur + 12.0:
                                                    probe_indices.append(idx)
                                            target_list = ready_indices if ready_indices else probe_indices
                                            if not target_list:
                                                break
                                            _cand_idx = target_list[0]
                                            _cand = _pending[_cand_idx]
                                            # 1. 视觉出点审计（函数内部先执行入点门禁；门禁可能把起点后移到真实 combat）
                                            clf = _rs_state.get('broadcast_classifier')
                                            if not isinstance(clf, ValorantFrameClassifier):
                                            ...
                                            try:
                                                ...
                                                # 超预算/被抢占时 cancel_check 会在审计内部抛
                                                # FFmpegCancelled。已定稿的拒绝结论必须照常交付
                                                # （契约：审计结果永不丢失），否则该候选每轮重扫
                                                # 一遍、永远停留在 pending，前端长期展示未审计的
                                                # 粗边界切片（实测 12 次超预算、0 条交付）。
                                                _deliver_audit_outcomes_on_cancel(
                                                    _audit_outcome_sink,
                                                    _pending,
                                                    _cand_idx,
                                                    produced_audited,
                                                    task_state,
                                                    current_duration=_dur,
                                                    broadcast=bridge.queue_broadcast,
                                                )
                                            ...
                                            #    又把 start_delta 清空”的契约断裂。
                                            if audit_outcomes:
                                                from lsc.analyzer.valorant_broadcast import BroadcastAuditOutcome
                                                _refined_outcomes: list[Any] = []
                                                for _outcome in audit_outcomes:
                                                    _status = str(getattr(_outcome, 'status', '') or '').lower()
                                                    _cand_out = getattr(_outcome, 'candidate', None)
                                                    if _status in {'accepted', 'manual_review'} and isinstance(_cand_out, dict):
                                                        _refined_list = _run_with_onnx_resource(
                                                            refine_valorant_round_boundaries,
                                                            [dict(_cand_out)],
                                                            _vp,
                                                            _ffmpeg,
                                                        ...
                                                        if _refined_list:
                                                            _cand_out = _refined_list[0]
                                                        _refined_outcomes.append(BroadcastAuditOutcome(
                                                            status=_status,
                                                            candidate=_cand_out,
                                                            reason=str(getattr(_outcome, 'reason', '') or ''),
                                                            retry_after_duration=getattr(_outcome, 'retry_after_duration', None),
                                                        ))
                                                    else:
                                                        _refined_outcomes.append(_outcome)
                                                audit_outcomes = _refined_outcomes
                                            # 单个候选经超长分裂可能一次返回多个 outcome：
                                            # 批量消费（首个终态弹出槽位，其余 accepted 入列，
                                            # 非终态子候选以自身 start 重新入 pending 续扫）。
                                            if audit_outcomes and _consume_broadcast_audit_outcome_batch(
                                                _pending,
                                                _cand_idx,
                                                audit_outcomes,
                                                produced_audited,
                                    ...
                                    _pov_refined = []
                                    for _candidate in _rounds:
                                        _pov_refined.extend(
                                            _run_with_onnx_resource(
                                                refine_valorant_round_boundaries,
                                                [_candidate],
                                                _vp,
                                                _ffmpeg,
                                    ...
                                    for _candidate in _pov_refined:
                                        _enqueue_refine_result(
                                            task_state,
                                            _candidate,
                                            outcome='accepted',
                                        )
                                    ...
                                    async with _refine_semaphore:
                                        with _analysis_jobs_lock:
                                            ...
                                            if (
                                                ...
                                                if st is not None:
                                                    ...
                                                    st['refine_requested_at'] = 0.0
                                                    st['refine_fairness_active'] = False
                                                return
                                            st['refine_pending'] = False
                                        ...
                                        try:
                                            ...
                                            def _run_refine_guarded():
                                                return _do_boundary_refine()

                                            _refine_fut = loop.run_in_executor(
                                                _ai_executor, _run_refine_guarded,
                                            )
                                            try:
                                                ...
                                                with _analysis_jobs_lock:
                                                    timeout_state = _continuous_tasks.get(room_id)
                                                ...
                                                try:
                                                    ...
                                                    # Future 仍在 executor 中结束时必须取走
                                                    # exception，否则 asyncio 会额外报
                                                    # "Future exception was never retrieved"。
                                                    _refine_fut.add_done_callback(
                                                        lambda done: done.exception()
                                                        if not done.cancelled()
                                                        else None
                                                    )
                                                    _log.error(
                                                        "边界审计中止后仍未退出: room_id=%s",
                                                        room_id,
                                                    )
                                                raise
                                            if _step_refined:
                                                refined.extend(_step_refined)
                                        finally:
                                            with _analysis_jobs_lock:
                                                st2 = _continuous_tasks.get(room_id)
                                                if st2 is not None:
                                                    st2['refine_running'] = False
                                                    st2['refine_requested_at'] = 0.0
                                                    st2['refine_fairness_active'] = False
                                except Exception as refine_exc:
                                    # 每个 accepted/manual_review 候选在审计函数返回
                                    # outcome 时已经写入可靠队列；异常只影响尚未完成的
                                    # 候选，不能以 executor 异常覆盖/清空已入队结果。
                                    _log.warning(
                                        "边界审计中止，保留已入队结果并等待主循环消费: room_id=%s, queued=%d, err=%r",
                                        room_id,
                                        len(_peek_refine_results(task_state)),
                                        refine_exc,
                                    )
                                    _publish_refine_results([])
                                    return
                                with _analysis_jobs_lock:
                                    st = _continuous_tasks.get(room_id)
                                    if (
                                        ...
                                        # refine_abort（粗扫抢占）只应中止未完成部分；
                                        # 已定稿回合仍须发布，避免抢占路径吞掉审计结果。
                                        if refined:
                                            _publish_refine_results(refined)
                                        return
                                _publish_refine_results(refined)
                            # 精修任务已经排队后，下一轮粗扫 kick 时通过 refine_abort
                            # 抢占它；扫描间隙内（粗扫完成后到下一 kick 之间）
                            # 审计正常推进，两路不再互相等待。
                            task_state['refine_pending'] = True
                            task_state['refine_requested_at'] = time.time()
                            task_state['coarse_windows_since_refine'] = 0
                            task_state['refine_fairness_active'] = _refine_fairness_due
                            # 扫描已完成：开放审计入口（扫描期间 refine_abort 保持 True）
                            task_state['refine_abort'] = False
                            _refine_task = asyncio.create_task(
                                _boundary_refine_bg(),
                                name=f"boundary-refine-{room_id[:8]}",
                            )
                            task_state['refine_task'] = _refine_task
                        elif _need_refine and not _allow_background_refine:
                            _log.info(
                                "持续分析滞后追赶期间暂缓边界审计: room_id=%s, backlog=%.1fs",
                                room_id,
                                _lag_now,
                            )
                        elif _need_refine and _refine_active:
                            # 审计在跑却被允许（lag 未超限、有 ready 候选）时不该出现"长期无进展"。
                            # 任务停在 semaphore/await 上会既没有微步骤超时日志、也没有结论交付，
                            # 表现为候选永久停在 pending（现场：21 分钟内 0 帧推理、0 交付）。
                            # 这里给一条节流告警，避免再次无迹可查。
                            _stall_anchor = float(
                                task_state.get('refine_requested_at')
                                or task_state.get('refine_started_at')
                                or 0.0
                            )
                            _stall_age = (
                                time.time() - _stall_anchor if _stall_anchor > 0.0 else 0.0
                            )
                            _last_stall_log = float(
                                task_state.get('_audit_stall_logged_at') or 0.0
                            )
                            if _stall_age >= 60.0 and time.time() - _last_stall_log >= 60.0:
                                task_state['_audit_stall_logged_at'] = time.time()
                                _log.warning(
                                    "边界审计任务 %.0fs 未推进（疑似卡死）: room_id=%s, "
                                    "pending=%d, running=%s, abort=%s, queue=%d",
                                    _stall_age,
                                    room_id,
                                    _bcast_pending_cnt,
                                    bool(task_state.get('refine_running')),
                                    bool(task_state.get('refine_abort')),
                                    int(task_state.get('refine_result_queue_depth') or 0),
                                )
                except Exception as exc:
                    # TimeoutError 的 str() 常为空，必须用 repr + exc_info；
                    # 但"换段/切换文件后读不到有效帧"（_ScanWindowRetryError）是**预期内**
                    # 的可重试状态，打完整 traceback 只会把真正的异常淹掉（2026-09-11 实测）。
                    _log.warning(
                    ...
                    scan_result_container['scan_succeeded'] = False
                    scan_result_container['video_path'] = video_path
                    scan_result_container['recording_id'] = scan_recording_id
            ...
            try:
                ...
                scan_result_container['recording_id'] = str(_st.get('recording_id') or '')
                scan_result_container['current_dur'] = float(_st.get('current_dur') or 0.0)
                scan_result_container['scan_range'] = tuple(
                    _st.get('scan_range') or (0.0, 0.0)
                )
                scan_result_container['completed_at'] = time.time()
    ...
    async def _continuous_analysis_loop(
        ...
        resume_job: FinalizationJob | None = None,
    ) -> None:
        """持续分析后台循环（生产者-消费者模式）。
        ...
        # 收尾分析状态：实时阶段不可信时使用 fresh full rescan，可靠时才续扫尾部
        ...
        _finalization_job: FinalizationJob | None = resume_job
        if resume_job is not None:
            # 恢复任务统一重新 fresh 扫描，实时 FSM 游标不作为收尾正确性的依据。
        ...
        def _get_recording_file_info():
            ...
            return None, 0.0
        def _ensure_finalization_plan(
            state: dict[str, Any],
            final_duration: float,
        ) -> None:
            """Freeze the finalization decision before the first finalize scan."""
            nonlocal _finalization_job
            if state.get('finalization_plan_ready'):
                return
            room = manager.get_room(room_id)
            source_path = str(video_path or state.get('video_path') or '')
            recording_id = str(
                getattr(room, 'recording_id', '') if room is not None else state.get('recording_id', '')
            )
            target_duration = max(0.0, float(final_duration or 0.0))
            job_id = f"finalize-{room_id}-{int(time.time() * 1000)}"
            _finalization_job = FinalizationJob.create(
                job_id=job_id,
                room_id=room_id,
                recording_id=recording_id,
                source_path=source_path,
                final_duration=target_duration,
                valorant_profile=str(state.get('valorant_profile') or 'pov'),
            )
            _cur_pending = (state.get('ocr_runtime_state') or {}).get('broadcast_pending_rounds') or []
            if _cur_pending:
                _finalization_job.update_pending_candidates(_cur_pending)
            for item in state.get('refine_result_queue') or []:
                if not isinstance(item, dict):
                    continue
                candidate = item.get('candidate')
                if isinstance(candidate, dict):
                    _finalization_job.enqueue_refine_result(
                        candidate,
                        str(item.get('delivery_key') or ''),
                        outcome=str(item.get('outcome') or 'accepted'),
                    )
            _finalization_job.accepted_candidates = [
                dict(item)
                for item in (state.get('accepted_candidates') or [])
                if isinstance(item, dict)
            ]
            _finalization_job.rejected_candidates = [
                dict(item)
                for item in (state.get('rejected_candidates') or [])
                if isinstance(item, dict)
            ]
            requires_full = finalization_requires_full_rescan(
                final_duration=target_duration,
                ...
                scan_error=bool(state.get('last_scan_error')),
            )
            state['finalization_plan_ready'] = True
            state['finalization_full_rescan'] = requires_full
            state['finalization_job_id'] = job_id
            state['finalization_job'] = _finalization_job.to_dict()
            if source_path:
                save_finalization_job(source_path, state['finalization_job'])
            _log.info(
                "持续分析收尾计划: room_id=%s, job=%s, full_rescan=%s, duration=%.1fs, last_analyzed=%.1fs",
                room_id,
                job_id,
                requires_full,
                target_duration,
        ...
        with _analysis_jobs_lock:
            ...
            # 新录制 epoch 隔离：上一会话的 listed_clips/键位登记若带进新录制，
            # 旧切片会混入权威快照与剪映草稿（recording_id 变化即新 epoch）。
            _prev_task_state = _continuous_tasks[room_id]
            _epoch_room = manager.get_room(room_id)
            _epoch_changed = _continuous_epoch_changed(
                _prev_task_state, _epoch_room,
                resume=resume_job is not None,
            )
            if _epoch_changed:
                _reset_epoch_scoped_clip_state(room_id, _prev_task_state)
                # 新录制 epoch：上一会话的终态权威快照必须一起清掉，
                # 否则旧回合会被当成"当前 epoch 权威集合"放行（C1 的清理口）。
                _clear_authority_snapshot(room_id)
            _continuous_tasks[room_id].update({
                ...
                'refine_fairness_active': False,
                'refine_task': None,
                'scan_done_event': scan_done_event,
                'room_id': room_id,
                'video_path': '',
                'recording_id': (
                    resume_job.recording_id
                    if resume_job is not None
                    else str(getattr(_epoch_room, 'recording_id', '') or '')
                ),
                # 新 epoch 清空旧会话切片快照；同 epoch 重启分析保留已入列切片
                'listed_clips': (
                    dict(_prev_task_state.get('listed_clips') or {})
                    if not _epoch_changed else {}
                ),
                # 精修结果独立于 scan_result_container，支持审计批超时、
                # refine_abort 或抽帧异常后的至少一次交付。
                'refine_result_queue': _resume_refine_result_queue(resume_job),
                'refine_delivered_keys': {},
                'refine_result_queue_depth': (
                    len(_resume_refine_result_queue(resume_job))
                    if resume_job is not None else 0
                ),
                ...
                'audit_accepted_count': (
                    sum(
                        1 for item in _resume_refine_result_queue(resume_job)
                        if isinstance(item, dict)
                        and str(item.get('outcome') or 'accepted') == 'accepted'
                    )
                    if resume_job is not None else 0
                ),
                'audit_manual_review_count': (
                    sum(
                        1 for item in _resume_refine_result_queue(resume_job)
                        if isinstance(item, dict)
                        and str(item.get('outcome') or '') == 'manual_review'
                    )
                    if resume_job is not None else 0
                ),
                ...
                'accepted_candidates': (
                    [dict(item) for item in resume_job.accepted_candidates if isinstance(item, dict)]
                    if resume_job is not None else []
                ),
                'rejected_candidates': (
                    [dict(item) for item in resume_job.rejected_candidates if isinstance(item, dict)]
                    if resume_job is not None else []
                ),
                'rejected_round_keys': (
                    {
                        _valorant_round_key(item): {
                            'start': item.get('start'),
                            'end': item.get('end'),
                            'reason': item.get('broadcast_start_gate')
                            or item.get('broadcast_audit')
                            or 'rejected',
                            'recorded_at': time.time(),
                        }
                        for item in resume_job.rejected_candidates
                        if isinstance(item, dict)
                    }
                    if resume_job is not None else {}
                ),
                ...
                'ocr_runtime_state': (
                    {'broadcast_pending_rounds': [dict(item) for item in resume_job.pending_candidates if isinstance(item, dict)]}
                    if resume_job is not None and resume_job.pending_candidates
                    else {}
                ),
                # 来源策略由 start_continuous_analysis 解析后传入；不得写死为
                # valorant，否则官方赛事无法启用 replay/暂停安全审计。
                'valorant_profile': str(valorant_profile or 'pov'),
                # S1: worker 崩溃重建计数（达到上限则终止任务，防止无限重建循环）
                'worker_restarts': 0,
                # Successful scan ledger. A single last_analyzed cursor is not
                # enough to prove finalization coverage after a timeout or file
                # switch.
                'coverage_ranges': (
                    [list(item) for item in resume_job.coverage_ranges]
                    if resume_job is not None else []
                ),
                'coverage_complete': bool(
                    resume_job is not None and resume_job.is_fully_covered()
                ),
                # 实际扫描周期用于追赶预算；不能用 Valorant 的 5s 轮询间隔替代。
                'scan_cycle_sec': float(max(5, interval)),
                'last_scan_started_mono': 0.0,
                'last_scan_wall_sec': 0.0,
                'scan_throughput_history': [],
                'net_coverage_throughput_history': [],
                'analysis_backlog_sec': 0.0,
                'realtime_fast_mode': False,
                'retry_scan_range': None,
                'retry_scan_depth': 0,
                'finalization_job_id': resume_job.job_id if resume_job is not None else None,
                'finalization_job': resume_job.to_dict() if resume_job is not None else None,
                'finalization_full_rescan': resume_job is not None,
                'finalization_full_rescan_started': False,
                'finalization_scan_cursor': 0.0,
                'finalize_boundary_pass': False,
                'finalization_plan_ready': resume_job is not None,
                'finalizing': resume_job is not None,
                'stop_requested': resume_job is not None,
            })
        ...
        def _spawn_worker():
            _w = asyncio.create_task(
                _continuous_valorant_worker(
                    room_id, mode, game, threshold,
                    _continuous_tasks, _analysis_semaphore,
                    _analysis_thread_semaphore, _bridge_executor,
                    scan_result,
                ),
        ...
        _log.info(
            "持续分析启动: room_id=%s, mode=%s, game=%s, profile=%s, interval=%ds, 增量回合窗口=%s",
            room_id, mode, game, valorant_profile or 'pov', interval,
            _valorant_incremental_rounds,
        )
        ...
        try:
            while True:
                ...
                # 当有足够的新录制媒体待分析时，缩短休眠，迅速追平录制沿（将滞后保持在 ≤30s）
                if _valorant_incremental_rounds and float(current_dur) > float(last_analyzed) + 12.0:
                    _sleep_time = min(_sleep_time, 3.0)
                # 精修结果是独立的可靠队列；不能只看单个 scan_result_container
                # 的时间戳，否则粗扫/精修完成竞态会吞掉其中一方。
                _refine_result_items = _peek_refine_results(state or {})
                _pending_result = bool(
                    float(scan_result.get('completed_at', 0.0) or 0.0) > last_consumed_at
                    or _refine_result_items
                )
                if not _pending_result:
                    try:
                        ...
                        _skip_wait = bool(
                            ...
                            or _peek_refine_results(_pre_sleep_state or {})
                        )
                        if not _skip_wait:
                ...
                if _path_changed and _last_video_path:
                    # 区分：是同一录制文件的收尾归档重命名（如 _录制中.mp4 -> _至_....mp4），还是真正的重连新文件
                    _old_base = os.path.basename(_last_video_path)
                    _new_base = os.path.basename(video_path)
                    _is_finalize_rename = False
                    if ("_录制中" in _old_base or "_in_progress" in _old_base) and (
                        "_录制中" not in _new_base and "_in_progress" not in _new_base
                    ) or not os.path.exists(_last_video_path) and os.path.exists(video_path) and current_dur >= last_analyzed:
                        _is_finalize_rename = True
                    if _is_finalize_rename:
                        _log.info(
                            "持续分析检测到录制文件收尾改名（保留分析进度）: room_id=%s, old=%s -> new=%s, 已分析=%.1fs/%.1fs",
                            room_id, _old_base, _new_base, last_analyzed, current_dur,
                        )
                        # 文件只是收尾更名，录制已停止，保留已分析进度并立即触发收尾
                        _recording_stop_ticks = 2
                        _file_switch_cooldown = 0
                        _finalize_pending = True
                        if _finalization_job is not None:
                            # 回写前从权威 state 重建，避免本地旧对象清掉 coverage 账本
                            _finalization_job = _finalization_job_from_state(state) or _finalization_job
                            _finalization_job.source_path = str(video_path)
                            _finalization_job.final_duration = max(
                                _finalization_job.final_duration,
                                float(current_dur or 0.0),
                            )
                            state['finalization_job'] = _finalization_job.to_dict()
                            if save_finalization_job(
                                _finalization_job.source_path,
                                state['finalization_job'],
                            ):
                                archive_finalization_job(_last_video_path)
                    else:
                        _log.info(
                            "持续分析检测到录制文件切换（新开录制）: room_id=%s, old=%s, new=%s, 重置分析游标",
                            room_id, _old_base, _new_base,
                        )
                        last_analyzed = 0.0
                        ...
                        with _analysis_jobs_lock:
                            if room_id in _continuous_tasks:
                                ...
                                _continuous_tasks[room_id]['coverage_ranges'] = []
                                _continuous_tasks[room_id]['coverage_complete'] = False
                                _continuous_tasks[room_id]['finalization_job_id'] = None
                                _continuous_tasks[room_id]['finalization_job'] = None
                                _continuous_tasks[room_id]['finalization_full_rescan'] = False
                                _continuous_tasks[room_id]['finalization_full_rescan_started'] = False
                                _continuous_tasks[room_id]['finalization_scan_cursor'] = 0.0
                                _continuous_tasks[room_id]['finalize_boundary_pass'] = False
                                _continuous_tasks[room_id]['finalization_plan_ready'] = False
                if video_path:
                    _last_video_path = video_path
                room_obj = manager.get_room(room_id)
                if room_obj is not None:
                    state['recording_id'] = str(
                        getattr(room_obj, 'recording_id', '')
                        or state.get('recording_id')
                        or ''
                    )
                ...
                _analysis_backlog = max(0.0, float(recorded_duration) - float(last_analyzed))
                state['analysis_backlog_sec'] = _analysis_backlog
                # backlog 失控时只降低中央横幅 OCR 频率；顶部计时器仍逐帧读取，
                # 收尾 fresh scan 永远关闭该快速模式。
                # broadcast（赛事/二路）模式以质量为最高优先级：即使长时间落后，
                # 也不进入 realtime_fast_mode（该模式会把候选全部推迟到收尾再审计，
                # 导致后续切片得不到逐回合视觉审计，质量下降）。
                _fast_threshold = (
                    180.0
                    if state.get('valorant_profile') == 'broadcast'
                    else 60.0
                )
                state['realtime_fast_mode'] = bool(
                    _valorant_incremental_rounds
                    and _analysis_backlog > _fast_threshold
                    and state.get('valorant_profile') != 'broadcast'
                ...
                if is_still_recording:
                    ...
                    # 冷却期每个 tick 都要递减，防止停录后 cooldown 永久卡住导致死锁
                    if _file_switch_cooldown > 0:
                        _file_switch_cooldown -= 1
                        _recording_stop_ticks = 0
                    else:
                ...
                if _finalize_started or _finalize_pending:
                    _is_bcast = state.get('valorant_profile') == 'broadcast'
                    state['analysis_stage'] = (
                        '收尾中（赛事转播视觉审计中，请勿关闭应用）'
                        if _is_bcast and state.get('scan_running')
                        else '收尾中'
                    )
                ...
                _scan_result_pending = (
                    float(scan_result.get('completed_at', 0.0) or 0.0)
                    > last_consumed_at
                )
                _refine_result_items = _peek_refine_results(state)
                _refine_candidates = [
                    dict(item['candidate'])
                    for item in _refine_result_items
                    if isinstance(item.get('candidate'), dict)
                ]
                _refine_delivery_keys = [
                    str(item.get('delivery_key') or '')
                    for item in _refine_result_items
                    if item.get('delivery_key')
                ]
                worker_completed_at = scan_result.get('completed_at', 0.0)
                worker_dur = scan_result.get('current_dur', 0.0)
                worker_result = (
                    list(scan_result.get('result', []) or [])
                    if _scan_result_pending else []
                ) + _refine_candidates
                # A refine-only wakeup must not advance last_analyzed or reuse
                # the coarse scan's old path metadata. It still enters the same
                # merge/list pipeline so round_key dedupe remains centralized.
                _refine_only_delivery = bool(_refine_candidates and not _scan_result_pending)
                if _refine_only_delivery:
                    worker_completed_at = max(
                        float(worker_completed_at or 0.0), time.time()
                    )
                    worker_dur = last_analyzed
                worker_error = (
                    scan_result.get('error')
                    if _scan_result_pending and not _refine_candidates
                    else None
                )
                _boundary_refine_pass = bool(
                    scan_result.get('boundary_refine_pass') or _refine_candidates
                )
                _time_since_last_consume = time.time() - last_consumed_at
                _min_consume_interval = 10.0 if _valorant_incremental_rounds else 0.0
                can_consume = (
                    (worker_completed_at > last_consumed_at or _refine_candidates)
                    and (
                ...
                # 录制文件切换守卫：在途扫描完成时若录制文件已切换（如网络重连），
                # 若扫出有效回合必须抢救入列，不可暴力丢弃；但不得将旧文件时长
                # 写回当前文件游标（避免反向区间）。
                _scanned_video = str(
                    scan_result.get('video_path') or (video_path if _refine_only_delivery else '')
                )
                _scanned_recording_id = str(
                    scan_result.get('recording_id') or (
                        getattr(room_obj, 'recording_id', '') if _refine_only_delivery else ''
                    )
                )
                _current_recording_id = str(getattr(room_obj, 'recording_id', '') or '')
                _stale_scan_result = bool(
                    (_scanned_recording_id and _current_recording_id
                ...
                if _stale_scan_result and not worker_result:
                    ...
                    # 到达，实测每 ~5s 刷一条、连续数分钟（run7 日志），纯噪音。
                    ...
                    with _analysis_jobs_lock:
                        if room_id in _continuous_tasks:
                            ...
                            if (_finalize_started or _finalize_pending) and _finalization_job is not None:
                                _failed_range = scan_result.get('scan_range') or (0.0, 0.0)
                                if isinstance(_failed_range, (list, tuple)) and len(_failed_range) >= 2:
                                    # 失败登记前从权威 state 重建，保留已覆盖账本
                                    _finalization_job = _finalization_job_from_state(state) or _finalization_job
                                    _finalization_job.add_failure(
                                        float(_failed_range[0]),
                                        float(_failed_range[1]),
                                        str(worker_error),
                                        attempt=_finalize_failures + 1,
                                    )
                                    _continuous_tasks[room_id]['finalization_job'] = _finalization_job.to_dict()
                                    save_finalization_job(
                                        _finalization_job.source_path,
                                        _continuous_tasks[room_id]['finalization_job'],
                                    )
                            # 失败保持退避，超时另行拆分窗口后重试。
                            ...
                            if (
                                ...
                                _retry = _schedule_scan_timeout_retry(
                                    _continuous_tasks[room_id],
                                    tuple(scan_result.get('scan_range') or (0.0, 0.0)),
                                    worker_error,
                                )
                                _log.warning(
                                    "持续分析超时，拆分窗口重试: room_id=%s, retry_range=%s",
                                    room_id,
                                    _retry,
                                )
                    if terminal_model_error:
                        ...
                        if _finalize_failures < _finalize_max_attempts:
                            ...
                            with _analysis_jobs_lock:
                                if room_id in _continuous_tasks:
                                    ...
                                    _continuous_tasks[room_id]['finalization_state'] = 'checkpoint_saved'
                            _log.warning(
                            ...
                            with _analysis_jobs_lock:
                                if room_id in _continuous_tasks:
                                    ...
                                    _continuous_tasks[room_id]['finalization_state'] = 'checkpoint_saved'
                                    _continuous_tasks[room_id]['completed'] = False
                                    _continuous_tasks[room_id]['cancelled'] = True
                                    _continuous_tasks[room_id]['finalize_error'] = worker_error
                                    if _finalization_job is not None:
                                        # 回写前从权威 state 重建，保留已覆盖账本
                                        _finalization_job = _finalization_job_from_state(state) or _finalization_job
                                        _finalization_job.phase = 'error'
                                        _finalization_job.last_error = str(worker_error)
                                        _continuous_tasks[room_id]['finalization_job'] = _finalization_job.to_dict()
                                        save_finalization_job(
                                            _finalization_job.source_path,
                                            _continuous_tasks[room_id]['finalization_job'],
                            ...
                            bridge.queue_broadcast({
                                ...
                                'data': {
                                    ...
                                    'listed_clip_count': len(_continuous_listed_clip_snapshot(state)),
                                    'listed_clips': _continuous_listed_clip_snapshot(state),
                                    'coverage_ranges': _continuous_coverage_snapshot(state),
                                    'coverage_complete': bool(state.get('coverage_complete')),
                                    'completed': False,
                                    'error': worker_error,
                                    # 收尾超时/扫描失败（非终端模型错误）放弃：pending 候选已
                                    # 落盘到 sidecar，可由 resume_continuous_finalization 恢复。
                                    # 前端据此权威标记渲染“恢复收尾”按钮，区分不可恢复的终端错误。
                                    'finalization_recoverable': True,
                                    'finalization_state': 'checkpoint_saved',
                                    'audit_delivery_gap': state.get('audit_delivery_gap', 0),
                                    'finalization_job_id': state.get('finalization_job_id'),
                                },
                            })
                    else:
                        _log.warning(
                    ...
                    with _analysis_jobs_lock:
                        if room_id in _continuous_tasks:
                            ...
                            if not _stale_scan_result:
                                _record_continuous_scan_coverage(
                                    _continuous_tasks[room_id],
                                    scan_result.get('scan_range'),
                                    final_duration=recorded_duration,
                                )
                    ...
                    if _valorant_incremental_rounds:
                        ...
                        _rejected_keys = set((state or {}).get('rejected_round_keys') or {})
                        window_rounds = (
                            [item for item in new_hl if _valorant_round_key(item) not in _rejected_keys]
                            if _finalize_started
                            else _drop_open_tail_rounds(
                                [item for item in new_hl if _valorant_round_key(item) not in _rejected_keys],
                                worker_dur,
                            )
                        )
                        ...
                        full_rounds = [
                            item for item in full_rounds
                            if _valorant_round_key(item) not in _rejected_keys
                        ]
                    ...
                    if publish_update or retry_pending_exports:
                        ...
                        ok, _, main_room_for_map, target_rooms_for_map = await asyncio.get_running_loop().run_in_executor(
                            _bridge_executor,
                    ...
                    with _analysis_jobs_lock:
                        if room_id in _continuous_tasks:
                            ...
                            # coverage 吞吐；旧 scan_throughput 暂保留为粗扫口径。
                            if mode == 'valorant_round':
                                ...
                                _continuous_tasks[room_id]['last_scan_wall_sec'] = max(
                                    0.0, _scan_wall
                                )
                                if _scan_out > _scan_in and _scan_wall > 0.0:
                                    _metrics = _continuous_scan_metrics(
                                        scan_in=_scan_in,
                                        scan_out=_scan_out,
                                        analyzed_before=float(
                                            _continuous_tasks[room_id].get('_scan_analyzed_before')
                                            or 0.0
                                        ),
                                        analyzed_after=float(worker_dur),
                                        recorded_before=float(
                                            _continuous_tasks[room_id].get('_scan_recorded_before')
                                            or 0.0
                                        ),
                                        recorded_after=max(float(recorded_duration), float(worker_dur)),
                                        scan_wall_sec=_scan_wall,
                                    )
                                    _continuous_tasks[room_id].update(_metrics)
                                    _thru = _metrics['gross_scan_throughput']
                                    _continuous_tasks[room_id]['last_scan_throughput'] = round(_thru, 2)
                                    ...
                                    _net_hist = list(
                                        _continuous_tasks[room_id].get('net_coverage_throughput_history')
                                        or []
                                    )
                                    _net_hist.append(_metrics['net_coverage_throughput'])
                                    _continuous_tasks[room_id]['net_coverage_throughput_history'] = _net_hist[-5:]
                            confirmed_total = sum(
                    ...
                    # 若存在收尾任务，将最新 pending rounds 同步并落盘防丢失
                    if _finalization_job is not None:
                        _rt_pending = (state.get('ocr_runtime_state') or {}).get('broadcast_pending_rounds') or []
                        # 这是权威运行时队列：审计 accepted/rejected 后即使为空，
                        # 也必须清除 sidecar 中旧的 pending，避免恢复时重复审计。
                        # 落盘前从权威 state 重建 job：本地对象不含运行时 coverage
                        # 账本，旧对象回写会清掉 coverage（收尾判定的死循环根因）。
                        _finalization_job = _finalization_job_from_state(state) or _finalization_job
                        _finalization_job.replace_pending_candidates(_rt_pending)
                        state['finalization_job'] = _finalization_job.to_dict()
                        save_finalization_job(_finalization_job.source_path, state['finalization_job'])
                    ...
                    if (
                        ...
                        # 终点接近文件尾仍不代表覆盖账本完整；例如 worker 使用了
                        # 虚高/虚低 probe 时，必须继续补扫而不是提前 completed。
                        coverage_complete = bool(state.get('coverage_complete'))
                        # C6：pending 队列刚清零不等于"所有已入列切片都有归属"。
                        # 现场 20:45:20 判定瞬间 round-000135 的结论刚在 20:45:17 出来，
                        # 队列已空但 listed 里的 135 还没有终态 → completed 提前成立，
                        # 导出时它被判"不在权威集合"。这里把「listed 无归属」也算作未收尾，
                        # 并复用同一处有界兜底（到上限落 manual_review 终态，不放宽门禁）。
                        _unterminal_listed = _listed_items_without_terminal(state)
                        pending_audit = bool(
                            (state.get('ocr_runtime_state') or {}).get(
                                'broadcast_pending_rounds'
                            )
                        ) or bool(_unterminal_listed)
                        # 交付完整性初始化：HEAD 原有此行，C6 改动中被误删，导致下面的
                        # `delivery_complete = delivery_complete and ...` 自引用 →
                        # UnboundLocalError（ruff F821 抓到）。这里补回。
                        delivery_complete = not bool(_peek_refine_results(state))
                        if _finalization_job is not None:
                            # 判定前从权威 state 重建 job：本地对象不含运行时
                            # coverage 账本，旧对象 is_fully_covered 恒 False 会把
                            # 已完整覆盖的收尾卡成无限补扫。
                            _finalization_job = _finalization_job_from_state(state) or _finalization_job
                            # 两份账本取并集：作业账本可能只记了最后一个收尾窗口
                            # （实测 job=[[214.86,253.37]] 而运行时=[[0.0,253.37]]），
                            # 只信 job 会让 coverage_complete 恒 False → 有界兜底
                            # （_FINALIZE_TAIL_STALL_MAX_ROUNDS）永远不计数 →
                            # 「继续补扫」无限循环、待审计候选永不定稿（2026-09-11 现场）。
                            _runtime_coverage = list(_continuous_coverage_snapshot(state))
                            if _runtime_coverage:
                                _finalization_job.coverage_ranges = merge_ranges(
                                    [
                                        list(item)
                                        for item in (
                                            list(_finalization_job.coverage_ranges)
                                            + _runtime_coverage
                                        )
                                    ]
                                )
                            coverage_complete = coverage_complete and (
                                _finalization_job.is_fully_covered()
                                or not uncovered_ranges(
                                    _runtime_coverage,
                                    0.0,
                                    float(_finalization_job.final_duration or 0.0),
                                )
                            )
                            delivery_complete = delivery_complete and not bool(
                                _finalization_job.refine_result_queue
                            )
                        if not coverage_complete or pending_audit or not delivery_complete:
                            _finalize_started = False
                            _finalize_pending = True
                            # 有界兜底计数：仅当 coverage/delivery 都已满足、只剩待审计
                            # 队列非空时累加——这类滞留是结构性的（后视窗口不足），
                            # 与「暂时还没补扫到」不同，不能无限等待。
                            if coverage_complete and delivery_complete and pending_audit:
                                _stall_rounds = int(
                            ...
                            if (
                                ...
                                # 文件已定格且反复无法定稿：强制定稿剩余候选，否则收尾
                                # 会永久停在「继续补扫」。已产出 accepted 的候选不受影响。
                                _stalled = [
                                    dict(c)
                                    for c in (
                                        (state.get('ocr_runtime_state') or {}).get(
                                ...
                                state.setdefault('ocr_runtime_state', {})[
                                    'broadcast_pending_rounds'
                                ] = []
                                for _cand in _stalled:
                                    _cand.setdefault(
                                        'broadcast_audit', 'rejected_finalize_stalled'
                                    )
                                    _record_terminal_candidate_state(state, _cand, 'rejected')
                                    for _lk in _prune_rejected_listed_clips(state, _cand):
                                        bridge.queue_broadcast({
                                            'type': 'clip_confirm_status',
                                            'data': {
                                                'room_id': str(_lk).split(':', 1)[0],
                                                'round_key': _valorant_round_key(_cand),
                                                'confirm_status': 'rejected',
                                                'start': round(float(_cand.get('start') or 0.0), 1),
                                                'end': round(float(_cand.get('end') or 0.0), 1),
                                                'reason': 'finalize_stalled',
                                            },
                                        })
                                if _stalled:
                                    state['audit_rejected_count'] = int(
                                        state.get('audit_rejected_count') or 0
                                    ) + len(_stalled)
                                # C6 兜底：listed 里有、却始终没拿到终态的回合同样到上限收口。
                                # 落 manual_review（不删除、不放宽门禁）：导出侧因此能给出
                                # END_NOT_FINAL / NEVER_AUDITED 这类可辨原因，而不是"不在权威集合"，
                                # 也避免"队列空 + listed 无归属"让收尾无限重跑。
                                _stalled_listed = _listed_items_without_terminal(state)
                                for _item in _stalled_listed:
                                    _record_terminal_candidate_state(state, _item, 'manual_review')
                                    _log.warning(
                                        "收尾补扫达到上限，仍未定稿的已入列切片落 manual_review: room_id=%s, round_key=%s",
                                        room_id, _valorant_round_key(_item),
                                    )
                            ...
                            _finalize_failures = 0
                            if _finalization_job is not None:
                                _finalization_job.phase = 'completed'
                                _finalization_job.final_round_count = len(all_highlights)
                                _finalization_job.candidate_count = len(all_highlights)
                                _finalization_job.last_error = ''
                                state['finalization_job'] = _finalization_job.to_dict()
                                save_finalization_job(
                                    _finalization_job.source_path,
                                    state['finalization_job'],
                                )
                            state['finalization_state'] = 'completed'
                            ...
                            with _analysis_jobs_lock:
                                if room_id in _continuous_tasks:
                                    _continuous_tasks[room_id]['completed'] = True
                                    _continuous_tasks[room_id]['finalizing'] = False
                                    _continuous_tasks[room_id]['coverage_complete'] = True
                                    _continuous_tasks[room_id]['cancelled'] = True
                with _analysis_jobs_lock:
                    state = _continuous_tasks.get(room_id)
                if not state or state.get('cancelled'):
                ...
                if video_path:
                    should_kick = False
                    # 停止前的尾部补扫（stop_tail_scan）：有界地多扫几窗直到
                    # 覆盖请求时刻的尾部，或达到窗口上限；完成后置 cancelled 退出，
                    # 避免录制仍在增长时追直播沿无休止。
                    if (
                        state
                        and state.get('stop_tail_scan')
                        and not (_finalize_pending or _finalize_started)
                    ):
                        _stop_target = float(state.get('stop_tail_target') or 0.0)
                        _stop_windows = int(state.get('stop_tail_windows') or 0)
                        _kick_dur_now = max(
                            float(current_dur or 0.0),
                            float(state.get('recorded_duration') or 0.0),
                        )
                        _tail_left = max(
                            0.0, _kick_dur_now - float(last_analyzed)
                        )
                        _tail_done = bool(
                            (_stop_target > 0.0 and float(last_analyzed) >= _stop_target - 3.0)
                            or _tail_left <= _VALORANT_KICK_AHEAD_SEC + 2.0
                        )
                        if _tail_done or _stop_windows >= _STOP_TAIL_MAX_WINDOWS:
                            if not state.get('scan_running') and not state.get('scan_requested'):
                                _log.info(
                                    "持续分析尾部补扫完成，停止: room_id=%s, analyzed=%.1fs, target=%.1fs, windows=%d",
                                    room_id,
                                    float(last_analyzed),
                                    _stop_target,
                                    _stop_windows,
                                ...
                                continue
                            # 在途扫描完成后立即停止（下一 tick 检查）
                            should_kick = False
                    if _finalize_pending and not _finalize_started:
                        # 有在途扫描时先不置 _finalize_started：等其完成后，下一 tick
                        # 再 kick 收尾。若实时阶段曾明显落后或 coverage 不完整，
                        # 收尾必须切换到 fresh full scan，不能只从 last_analyzed 续扫。
                        if not state.get('scan_running'):
                            _ensure_finalization_plan(state, max(recorded_duration, current_dur))
                            should_kick = True
                            _finalize_started = True
                            with _analysis_jobs_lock:
                                if room_id in _continuous_tasks:
                                    _continuous_tasks[room_id]['finalizing'] = True
                                    _continuous_tasks[room_id]['finalization_state'] = 'finalizing'
                            if _finalization_job is not None:
                                # 回写前从权威 state 重建，保留已覆盖账本
                                _finalization_job = _finalization_job_from_state(state) or _finalization_job
                                _finalization_job.phase = 'finalizing'
                                state['finalization_job'] = _finalization_job.to_dict()
                                save_finalization_job(
                                    _finalization_job.source_path,
                                    state['finalization_job'],
                    ...
                    # 粗扫与审计已解耦（A-05）：审计在独立 _refine_semaphore 上运行；
                    # 是否抢占由 worker 内按 backlog 实时判定，主循环不主动掐审计。
                    if (
                        should_kick
                        and not state.get('scan_running')
                    ):
                        state['last_progress_broadcast_at'] = time.time()
                        _scan_counter += 1
                        _kick_started_mono = time.monotonic()
                        _previous_kick_mono = float(
                            state.get('last_scan_started_mono', 0.0) or 0.0
                        )
                        if _previous_kick_mono > 0.0:
                            state['scan_cycle_sec'] = max(
                                1.0,
                                _kick_started_mono - _previous_kick_mono,
                            )
                        state['last_scan_started_mono'] = _kick_started_mono
                        # 扫描预算：单点 _select_scan_window 入口，消除兼容预算二次覆盖
                        from lsc.analyzer.registry import get as get_analyzer
                        _analyzer = get_analyzer(game)
                        _retry_range = state.get('retry_scan_range')
                        # 自适应 lookback：失败/超时/文件切换冷却期用 30s 兜底，
                        # 稳态正常增量用 8s（窗口净吞吐提升的主要来源）。
                        _need_failed_lookback = bool(
                            _retry_range
                        ...
                        _plan_state = {
                            ...
                            'incremental_lookback': (
                                ...
                                if _need_failed_lookback
                                else _VALORANT_STEADY_LOOKBACK_SEC
                            ),
                            # 吞吐历史 + 实际扫描周期：自适应追赶上限输入。
                            # effective_interval 只是轮询节拍，不能代表 Worker 的处理耗时。
                            'throughput_history': _continuous_throughput_history(state),
                            'kick_interval': effective_interval,
                            'scan_cycle_sec': float(
                                state.get('scan_cycle_sec')
                                or state.get('last_scan_wall_sec')
                        ...
                        if not (_finalize_started or _finalize_pending) and not full_rescan:
                            ...
                            if scan_range[1] - scan_range[0] > 120.0:
                                scan_range = (scan_range[0], round(scan_range[0] + 120.0, 3))
                                _scan_timeout = min(_scan_timeout, _window_scan_timeout(120.0, use_ocr=use_ocr_this_tick))
                            # 停止补扫定向窗口：直接向停止时刻冻结的 target 推进，单窗
                            # 上限 _STOP_TAIL_WINDOW_CAP_SEC（与全局单窗保护一致），不再
                            # 套用自适应追赶预算（90s cap），显著缩短停止收尾时长。
                            if (
                                state
                                and state.get('stop_tail_scan')
                                and not (_finalize_pending or _finalize_started)
                            ):
                                _stop_target_win = float(state.get('stop_tail_target') or 0.0)
                                if _stop_target_win > 0.0:
                                    _stop_end_win = min(
                                        _stop_target_win,
                                        scan_range[0] + _STOP_TAIL_WINDOW_CAP_SEC,
                                    )
                                    if _stop_end_win > scan_range[0] + 1.0:
                                        scan_range = (
                                            scan_range[0],
                                            round(_stop_end_win, 3),
                                        )
                                        _scan_timeout = min(
                                            _scan_timeout,
                                            _window_scan_timeout(
                                                _stop_end_win - scan_range[0],
                                                use_ocr=use_ocr_this_tick,
                                            ),
                                        )
                        if _finalize_started or _finalize_pending:
                            _fresh_finalize = bool(
                                state.get('finalization_full_rescan')
                                and not state.get('finalization_full_rescan_started')
                            )
                            _is_bcast = state.get('valorant_profile') == 'broadcast'
                            if _fresh_finalize:
                                _finalize_target = max(
                                ...
                                _finalize_end = min(
                                    _finalize_target,
                                    _FINALIZATION_CHUNK_SEC,
                                )
                                scan_range = (0.0, _finalize_end)
                                use_ocr_this_tick = True
                                full_rescan = True
                                _scan_reason = 'finalize_full_chunk'
                                state['finalization_full_rescan_started'] = True
                                state['finalization_scan_cursor'] = 0.0
                                state['finalize_boundary_pass'] = bool(
                                    _finalize_end >= _finalize_target - _OCR_FINALIZE_TAIL_EPS
                                )
                                state['finalization_scan_started_at'] = time.time()
                                _scan_timeout = max(
                                    _scan_timeout,
                                    _finalize_scan_timeout(
                                        max(1.0, _finalize_end),
                                        attempt=_finalize_failures + 1,
                                        is_broadcast=_is_bcast,
                                    ),
                                )
                                # 保留 broadcast_pending_rounds 候选，避免重扫失败后已发现的回合丢失
                                _saved_pending = []
                                if isinstance(state.get('ocr_runtime_state'), dict):
                                    _saved_pending = list(state['ocr_runtime_state'].get('broadcast_pending_rounds') or [])
                                state['ocr_runtime_state'] = {}
                                if _saved_pending:
                                    state['ocr_runtime_state']['broadcast_pending_rounds'] = _saved_pending
                                if _finalization_job is not None:
                                    # 回写前从权威 state 重建，保留已覆盖账本
                                    _finalization_job = _finalization_job_from_state(state) or _finalization_job
                                    _finalization_job.source_path = str(video_path)
                                    ...
                                    _finalization_job.replace_pending_candidates(_saved_pending)
                                    state['finalization_job'] = _finalization_job.to_dict()
                                    save_finalization_job(
                                        _finalization_job.source_path,
                                        state['finalization_job'],
                                ...
                                _log.info(
                                    "持续分析启动 fresh full 收尾分片: room_id=%s, range=0-%.1fs, target=%.1fs, timeout=%ds",
                                    room_id,
                                    float(_finalize_end),
                                    float(_finalize_target),
                                    _scan_timeout,
                                )
                            elif state.get('finalization_full_rescan'):
                                # fresh full 的后续分片：按已覆盖游标向前推进，
                                # 保留重叠供 last_processed_ts/FSM 去重，避免再次
                                # 扫描 [0, target] 的单个黑盒任务。
                                _finalize_target = max(
                                    float(recorded_duration or 0.0),
                                    float(current_dur or 0.0),
                                ...
                                _cursor = max(
                                    float(state.get('finalization_scan_cursor') or 0.0),
                                    float(last_analyzed or 0.0),
                                )
                                _fin_start = max(0.0, _cursor - _FINALIZATION_CHUNK_OVERLAP_SEC)
                                _fin_end = min(
                                    _finalize_target,
                                    _cursor + _FINALIZATION_CHUNK_SEC,
                                )
                                if _fin_end <= _cursor + 0.001:
                                    _fin_start = _cursor
                                    _fin_end = min(
                                        _finalize_target,
                                        _cursor + _FINALIZATION_CHUNK_SEC,
                                    )
                                scan_range = (_fin_start, _fin_end)
                                use_ocr_this_tick = True
                                full_rescan = True
                                _scan_reason = 'finalize_full_chunk'
                                state['finalize_boundary_pass'] = bool(
                                    _fin_end >= _finalize_target - _OCR_FINALIZE_TAIL_EPS
                                )
                                _scan_timeout = max(
                                    _scan_timeout,
                                    _finalize_scan_timeout(
                                        max(1.0, _fin_end - _fin_start),
                                        attempt=_finalize_failures + 1,
                                        is_broadcast=_is_bcast,
                                    ),
                                )
                                ...
                                _log.info(
                                    "持续分析继续 fresh full 收尾分片: room_id=%s, range=%.1f-%.1f, target=%.1f, boundary_pass=%s",
                                    room_id,
                                    float(_fin_start),
                                    float(_fin_end),
                                    float(_finalize_target),
                                    state['finalize_boundary_pass'],
                                )
                            else:
                                # 停录收尾：按 coverage ledger 的真实缺口逐片续扫，
                                # 每片最多 _FINALIZATION_CHUNK_SEC，避免整段尾部一次
                                # 黑盒扫描超时，也不再无条件从 last_analyzed 回退 120s
                                # 造成已覆盖区间被重复大范围扫描。
                                full_rescan = False
                                _finalize_target = max(
                                    float(recorded_duration or 0.0),
                                ...
                                _real_gaps = uncovered_ranges_for_state(
                                    state,
                                    _finalize_target,
                                )
                                if _real_gaps:
                                    _gap_start = max(0.0, float(_real_gaps[0][0]))
                                    _gap_end = min(
                                        _finalize_target,
                                        float(_real_gaps[0][1]),
                                    )
                                    _fin_start = max(
                                        0.0,
                                        _gap_start - _FINALIZATION_CHUNK_OVERLAP_SEC,
                                    )
                                    _fin_end = min(
                                        _finalize_target,
                                        _gap_start + _FINALIZATION_CHUNK_SEC,
                                    )
                                    if _fin_end <= _gap_start + 0.001:
                                        _fin_start = _gap_start
                                        _fin_end = _gap_end
                                else:
                                    # coverage 账本无缺口但收尾 FSM 仍被触发时，只补扫
                                    # 最近一段小尾窗，避免整个历史被再次回扫。
                                    _fin_start = max(
                                        0.0,
                                        float(last_analyzed) - _FINALIZATION_CHUNK_OVERLAP_SEC,
                                    )
                                    _fin_end = min(
                                        _finalize_target,
                                        _fin_start + _FINALIZATION_CHUNK_SEC,
                                    )
                                if float(last_analyzed) > float(current_dur):
                                    _log.warning(
                                        "持续分析收尾游标越界，按尾部小窗收尾: room_id=%s, last_analyzed=%.1fs > dur=%.1fs",
                                        room_id,
                                        float(last_analyzed),
                                        float(current_dur),
                                    )
                                    _fin_start = max(
                                        0.0,
                                        float(current_dur) - _FINALIZATION_CHUNK_SEC,
                                    )
                                    _fin_end = float(current_dur)
                                state['finalize_boundary_pass'] = bool(
                                    _fin_end >= _finalize_target - _OCR_FINALIZE_TAIL_EPS
                                )
                                scan_range = (_fin_start, _fin_end)
                        ...
                        # 顶部 HUD 恒 1fps：ocr_sample_interval 不得作为降频旋钮
                        # （降低它会同时漏掉顶部计时器跳变与短横幅，伤回合边界证据）。
                        # 降本交给中央横幅哨兵间隔 center_sentinel_sec。
                        try:
                            _budget_ocr_iv = min(
                                1.0,
                                float(pressure.get('ocr_sample_interval', 1.0) or 1.0),
                            )
                        except (TypeError, ValueError):
                            _budget_ocr_iv = 1.0
                        state['ocr_sample_interval'] = _budget_ocr_iv
                        _lag_now_budget = max(
                            0.0, float(recorded_duration) - float(last_analyzed)
                        )
                        if _finalize_started or _finalize_pending:
                            state.setdefault('ocr_runtime_state', {})['center_sentinel_sec'] = 2.0
                        elif state.get('valorant_profile') == 'broadcast':
                            # 官方解说分支质量优先：长时间滞后也不放大中央哨兵间隔，
                            # 避免后续回合因中央横幅采样变稀疏导致边界/回合判定变差。
                            state.setdefault('ocr_runtime_state', {})['center_sentinel_sec'] = 4.0
                        elif _lag_now_budget > 180.0:
                            state.setdefault('ocr_runtime_state', {})['center_sentinel_sec'] = 8.0
                        elif _lag_now_budget > 60.0:
                            state.setdefault('ocr_runtime_state', {})['center_sentinel_sec'] = 6.0
                        else:
                            state.setdefault('ocr_runtime_state', {})['center_sentinel_sec'] = 4.0
                        ...
                        state['_scan_analyzed_before'] = float(last_analyzed)
                        state['_scan_recorded_before'] = float(recorded_duration)
                        ...
                        state['scan_reason'] = _scan_reason
                        _backlog_before = max(0.0, float(current_dur) - float(last_analyzed))
                        _planned_new_media = max(0.0, float(scan_range[1]) - float(last_analyzed))
                        _tp_hist = _continuous_throughput_history(state)
                        _tp_avg = round(sum(_tp_hist) / len(_tp_hist), 2) if _tp_hist else None
                        _scan_cycle = float(
                            state.get('scan_cycle_sec')
                            or state.get('last_scan_wall_sec')
                            or effective_interval
                        ...
                        _planner_name = (
                            'timeout_retry'
                            if _scan_reason == 'timeout_retry'
                            else (
                                'plugin_adaptive'
                                if hasattr(_analyzer, 'plan_scan_window') and getattr(_analyzer.capabilities(), 'realtime_continuous', False)
                                else 'fallback_budget'
                            )
                        )
                        from lsc.analyzer.valorant_plugin import decide_backlog_policy
                        _bcast_cnt = len((state.get('ocr_runtime_state') or {}).get('broadcast_pending_rounds') or [])
                        _backlog_mode, _ = decide_backlog_policy(
                            _backlog_before, _tp_avg, _bcast_cnt,
                            source_profile=state.get('valorant_profile', 'pov'),
                        )
                        state['planner'] = _planner_name
                        state['planned_new_media_sec'] = round(_planned_new_media, 1)
                        state['backlog_sec'] = round(_backlog_before, 1)
                        state['audit_queue_depth'] = _bcast_cnt
                        state['backlog_mode'] = _backlog_mode
                        # 仅在真正提交 kick 后清除已取出的重试范围；失败时由
                        # _schedule_scan_timeout_retry 写入下一次更小的范围。
                        if state.get('retry_scan_range') is not None:
                            state['retry_scan_range'] = None
                        bridge.queue_broadcast({
                            'type': 'continuous_analysis_status',
                            'data': {
                                'running': True,
                                ...
                                'analysis_stage': state.get('analysis_stage', '分析中'),
                                'total_highlights': len(all_highlights),
                                'phase': 'stopping' if (state.get('stop_requested') or state.get('stop_tail_scan')) else ('finalizing' if _finalize_started else 'running'),
                                'updated_at': time.time(),
                        ...
                        if not state.get('scan_running'):
                            if state.get('stop_tail_scan'):
                                with _analysis_jobs_lock:
                                    _continuous_tasks[room_id]['stop_tail_windows'] = int(
                                        _continuous_tasks[room_id].get('stop_tail_windows') or 0
                                    ) + 1
                            _log.info(
                                "持续分析 kick worker: room_id=%s, dur=%.0fs, range=%.1f-%.1f, reason=%s, backlog=%.1fs, new_media=%.1fs, throughput_avg=%s, cycle=%.1fs, planner=%s, OCR=%s, full=%s, finalize=%s",
                                room_id,
                                current_dur,
                                scan_range[0],
                                scan_range[1],
            ...
            if _worker_task and not _worker_task.done():
                try:
                    ...
                    pass
            # 边界精修是 worker 派生的后台 task，不能只等待粗扫 worker；
            # 否则停止后它仍可能占用唯一分析槽或把旧结果写回共享容器。
            _refine_task = stop_state.get('refine_task')
            if _refine_task is not None and not _refine_task.done():
                with _analysis_jobs_lock:
                    if room_id in _continuous_tasks:
                        _continuous_tasks[room_id]['refine_abort'] = True
                try:
                    await asyncio.wait_for(
                        asyncio.shield(_refine_task),
                        timeout=_SCAN_ABORT_GRACE_SEC,
                    )
                except asyncio.TimeoutError:
                    _log.warning(
                        "边界精修未在 %.1fs 内退出，继续等待至硬上限 %.1fs: room_id=%s",
                        _SCAN_ABORT_GRACE_SEC,
                        _SCAN_ABORT_HARD_SEC,
                        room_id,
                    )
                ...
                bridge.queue_broadcast({
                    ...
                    'data': {
                        **_base_terminal,
                        ...
                        'analysis_stage': '收尾失败',
                        'total_highlights': len(all_highlights),
                        # 收尾超时/扫描失败放弃（非终端模型错误）：pending 候选已落盘，
                        # 前端据此权威标记渲染“恢复收尾”按钮（调 resume_continuous_finalization）。
                        'finalization_recoverable': True,
                        'finalization_state': 'checkpoint_saved',
                        'audit_delivery_gap': stop_state.get('audit_delivery_gap', 0),
                        'finalization_job_id': stop_state.get('finalization_job_id'),
                    },
                })
                ...
                bridge.queue_broadcast({
                    ...
                    'data': {
                        **_base_terminal,
                        'phase': 'idle',
                        'status': 'idle',
                        # 立即停止（仅停止分析）不产生 completed 收尾态；显式给出
                        # idle/finalization_state，前端才能据此触发“当前已入列切片”的草稿。
                        'finalization_state': 'idle',
                        'coverage_complete': bool(stop_state.get('coverage_complete', False)),
                        'audit_delivery_gap': stop_state.get('audit_delivery_gap', 0),
                        'pending_queue_depth': stop_state.get('audit_queue_depth', 0),
                        'target_room_ids': stop_state.get('target_room_ids') or [],
                    },
                })
            # C1：pop 之前把终态权威快照留下。收尾完成后用户随时可能导出/生成草稿，
            # 此时任务态已不在，只有 sidecar 可读；sidecar 可能落后于扫描通路
            # （本例 20:37 的文件 vs 20:43 的结论），没有快照就会把已定稿切片改回 pending。
            _preserve_authority_snapshot(room_id, stop_state)
            with _analysis_jobs_lock:
    ...
    async def _export_and_broadcast(
        ...
        mapping_error = ""
        # wait_for_file=True 内部 time.sleep 轮询最长 8s，必须放 executor，
        # 否则整个 WS 事件循环冻结、所有房间消息停摆
        ok, error, main_room_for_map, target_rooms_for_map = await asyncio.get_running_loop().run_in_executor(
            _bridge_executor,
            lambda: _validate_synced_analysis_targets(
                manager, main_room_id, target_room_ids, wait_for_file=True,
            ),
        ...
        if video_path:
            with _analysis_jobs_lock:
                stop_state = _continuous_tasks.get(room_id, {})
            # 终态落盘前再次过滤 rejected tombstone，防止强停/收尾路径把已拒绝
            # 候选写回 analysis sidecar 并在恢复导出时复活。
            _rejected_keys = set((stop_state or {}).get('rejected_round_keys') or {})
            if _rejected_keys:
                all_highlights = [
            ...
            # C3：落盘路径必须跟住归档改名。现场 20:38:42 录像改名（sidecar 同步改名），
            # 20:39:27 这次落盘仍写回旧名 `…_录制中.analysis.json`，归档文件的 sidecar
            # 被冻在改名前的 20:37 快照（缺 round-000135），导出时 135 被判"不在权威集合"。
            _save_path = _sync_analysis_save_path(manager, room_id, video_path)
            save_analysis_results(
                _save_path, room_id, mode, all_highlights,
    ...
    async def handle_delete_clip(data):
        ...
        with _analysis_jobs_lock:
            ...
            for job_state in _analysis_jobs.values():
                ...
                if snapshots and listed_key in snapshots:
                    snapshots.pop(listed_key, None)
        # 解除精修冻结与残留状态，避免引用已删切片（room 作用域 + 裸键兼容）
        with _refined_round_keys_lock:
    ...
    register_analysis_handlers(
        ...
        analysis_thread_semaphore=_analysis_thread_semaphore,
        load_settings=load_settings,
    ...
    register_jianying_handlers(
        ...
        manager=manager,
        load_settings=load_settings,
        # 权威注册表必须注入：默认空 dict 会让 listed_clips 权威补全与终态快照
        # 静默失效（草稿只剩 sidecar 回落，105 那类"已定稿却丢"就是这么丢的）。
        continuous_tasks=_continuous_tasks,
        analysis_jobs=_analysis_jobs,
        authority_snapshots=_last_authority_snapshots,
    )
